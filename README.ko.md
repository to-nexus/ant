...
<p align="center">
  ...
  혹은 <b>업무 에이전트를 파일로 직접 정의</b>해 같은 런타임 위에서 돌립니다.<br>
  <sub>프론트엔드 · 백엔드 · 확장 가능한 언어/프레임워크 tier · 서비스 연결 & mock 가상화 · 피처별 dev server · 브라우저 IDE · MCP 기반 커스텀 에이전트 (실험적) · 배포 (매니지드 클라우드)</sub>
</p>
...
위 내용은 전부 **코드스페이스** — 소프트웨어를 만드는 프로젝트 이야기입니다.
프로젝트에는 두 번째 종류가 있고, 첫 번째의 축소판이 아닙니다:
에이전트를 직접 정의하는 **워크스페이스**입니다.

<p align="center">
  <img src="docs/assets/codespace-workspace.png" width="880"
       alt="하나의 런타임 위에 놓인 두 종류의 공간 — 왼쪽 코드스페이스는 plan·design·code·visual·learn·ask 잡과 git 앵커, 피처 worktree, 칸반 보드를 갖고, 오른쪽 워크스페이스는 파일로 정의된 커스텀 잡만 노출하며 universal artifacts·sessions 트리와 체크리스트 보드를 갖고, 둘이 projectType 은 정책만 결정하고 레이아웃은 건드리지 않는 하나의 런타임으로 수렴하는 다이어그램">
</p>

|                          | **코드스페이스**                              | **워크스페이스** *(실험적)*                      |
|--------------------------|----------------------------------------------|------------------------------------------------|
| 만드는 것                 | 제품                                          | 조직의 업무 에이전트                             |
| 노출되는 잡               | `plan` `design` `code` `visual` `learn` `ask` | 파일로 정의한 당신의 잡                          |
| 작업 단위                 | **피처** — 브랜치 + worktree                  | **(에이전트, 잡)** 페어                          |
| 진행 표면                 | verification 태스크가 게이팅하는 칸반 태스크    | 에이전트가 직접 쓰는 체크리스트                   |
| git · 라이브 프리뷰 · 브라우저 IDE | 있음                                | 없음 — 워크스페이스에는 코드베이스가 없습니다      |
| 디스크 상                 | `repo.git` + `features/{feature}/…`          | `universal/{artifacts,sessions}/`              |

종류는 생성 시점에 정해지고 프로젝트 수명 동안 고정됩니다 — `config.json`의
`projectType`에 저장됩니다. 양방향으로 엄격한 분할이라서(두 종류에서 동시에
도는 잡 타입은 없습니다) 토글이 아니라 생성 시점 결정입니다.

포크는 아닙니다: 두 종류는 같은 4개 프로세스, 같은 Redis 버스, 같은 에이전트
루프 위에서 돕니다. `projectType`은 프로젝트가 어떤 잡을 노출하는지만
결정하며, 그 외에는 아무것도 바꾸지 않습니다.

더 읽기: [docs/concepts/spaces.md](docs/concepts/spaces.md).
...
다른 쪽이 먼저 궁금하다면, 같은 위저드에서 코드스페이스 대신
**워크스페이스**를 만들고 기본 제공되는 `assistant` 에이전트와 대화해 보세요 —
추가 셋업이 필요 없고, 직접 만들 때의 예제 역할도 합니다
([커스텀 에이전트](#커스텀-에이전트와-유니버설-런타임)).
...
도메인은 *코드스페이스*가 무엇을 만드는지를 규정합니다. 워크스페이스는 구조상
도메인이 없습니다 — 무엇을 하는지는 레지스트리 항목이 아니라 당신이 작성한
에이전트가 결정합니다.
...
제공되는 `assistant` 에이전트는 읽기 전용 예제로 들여다볼 수 있습니다.
...
| 워크스페이스 & 커스텀 에이전트 (`universal` 런타임) | **Experimental** | 런타임, 정의 로더, 암호화 크레덴셜 저장소를 갖춘 MCP 오버레이, 체크리스트 보드 모두 출하돼 있고 테스트가 있습니다. HTTP MCP 서버를 상대로 하는 읽기 전용 업무가 검증된 경로입니다(end-to-end 확인 완료). 아직 **없는 것 세 가지**: **대화형 승인** — 게이트에 걸린 쓰기 도구는 안내와 함께 거부되므로, 잡은 `approval: never` 를 명시한 곳에만 씁니다; **팀 공유** — 정의는 계정 소유이고 조직 스코프는 읽기 전용 디렉터리 하나뿐이라 모든 산출물이 개인 프로젝트에 놓입니다; **스케줄링** — 무인 실행 경로가 없습니다. 그리고 MCP 이미지 결과는 버려지고(텍스트 전용), 중단된 실행은 체크리스트 한 턴을 잃을 수 있습니다. |
| 팀 / 조직 공유 | **미출시** | 계정 전환은 동작합니다. 팀 생성과 초대는 동작하지 않습니다. |
