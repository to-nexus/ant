...
describe('deriveCookieDomain', () => {
  ...
  it('returns .cross.nexus for any *.cross.nexus host in production', () => {
    expect(deriveCookieDomain('cross.nexus', true)).toBe('.cross.nexus');
    expect(deriveCookieDomain('ant-server.cross.nexus', true)).toBe('.cross.nexus');
    expect(deriveCookieDomain('ant-preview.cross.nexus', true)).toBe('.cross.nexus');
    expect(deriveCookieDomain('ant.cross.nexus', true)).toBe('.cross.nexus');
  });
...
// NOTE: the `/auth/signout` dual-clear route-level test moved to the cloud overlay
// (packages/cloud/tests/http/signout-dual-clear.test.ts) along with createAuthRoutes.
// This OSS file keeps only the JwtService cookie-domain-derivation unit tests.
