...
describe('per-model token attribution', () => {
  it('buckets under the explicit per-node modelId, not the job default', () => {
    const state = makeState('claude-opus-5');
    ...
    expect(state.tokenUsageByModel?.['claude-opus-5']).toBeUndefined();
  });
  ...
  it('falls back to the job-default model when no modelId is supplied', () => {
    ...
    expect(state.tokenUsageByModel?.['claude-opus-5']).toMatchObject({ inputTokens: 10, outputTokens: 5 });
  });
  ...
  it('keeps separate buckets per model across a mixed-model job', () => {
    ...
    expect(state.tokenUsageByModel?.['claude-opus-5']).toMatchObject({ inputTokens: 50, outputTokens: 20, callCount: 1 });
...
describe('per-task per-model delta symmetry', () => {
  ...
  it('sum of per-task per-model deltas equals the job-level per-model map', () => {
    ...
    expect(state.tokenUsageByModel?.['claude-opus-5']).toMatchObject({ inputTokens: 147, outputTokens: 15 });
