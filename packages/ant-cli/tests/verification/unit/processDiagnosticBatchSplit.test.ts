...
describe('processDiagnosticBatchSplit — LLM-explicit fan-out', () => {
  ...
  describe('flat-plan size gate (dim-beating-brass) — force re-partition or soft-fail', () => {
    // A wide aggregator flat plan (7 feature domains) under a tight
    // recursion budget. Reproduces track-parent: 16 entries, post-
    // investigation budget. recursionCount is set high so the budget
    // condition trips deterministically.
    function wideAggregatorPlan(): string {
      const targets = [
      ...
      return JSON.stringify({
        task: { id: 'track-parent', goal: 'parent read-only screens across domains' },
        implementation: { modify: targets.map((t) => ({ target: t, action: 'wire read-only view' })) },
      });
    }

    it('feature + wide flat plan + tight budget → throws FlatPlanTooLargeViolation carrying the flat plan', () => {
      const state = makeState();
      state.recursionLimit = 200;
      state.recursionCount = 120; // remaining 80; est 16*6=96 > 48 → trip
      const task = makeTask({ type: 'feature', priority: 550, name: 'Route Track: 학부모 화면' });
      const plan = wideAggregatorPlan();
      let thrown: unknown;
      try {
        processDiagnosticBatchSplit(state, plan, task);
      } catch (e) {
      ...
      expect(thrown).toBeInstanceOf(FlatPlanTooLargeViolation);
      const v = thrown as FlatPlanTooLargeViolation;
      expect(v.detail.flatPlanText).toBe(plan);
      expect(v.detail.topLevelImplCount).toBe(16);
      expect(v.detail.distinctTopLevelDomains).toBeGreaterThanOrEqual(4);
      // No queue mutation on a reframe throw.
      expect(state.taskQueue.size()).toBe(0);
      expect(state._batchSplitRequeued).toBeFalsy();
    });

    it('reframe budget exhausted → soft-fail: marks task + throws VerificationTerminalError(flatplan_too_large)', () => {
      const state = makeState();
      state.recursionLimit = 200;
      state.recursionCount = 120;
      const task = makeTask({
        type: 'feature',
        priority: 550,
        name: 'Route Track: 학부모 화면',
      });
      (task as any)._flatPlanReframeCount = MAX_FLATPLAN_REFRAME_ATTEMPTS; // budget spent
      const plan = wideAggregatorPlan();
      let thrown: unknown;
      try {
        processDiagnosticBatchSplit(state, plan, task);
      ...
      expect((task as any)._failureReason).toMatch(/single session/i);
    });

    it('soft-fail reason is Korean when userLanguage=ko', () => {
      const state = makeState();
      state.recursionLimit = 200;
      state.recursionCount = 120;
      state.context = { featurePath: undefined, userLanguage: 'ko' };
      const task = makeTask({ type: 'feature', priority: 550, name: '학부모 트랙' });
      (task as any)._flatPlanReframeCount = MAX_FLATPLAN_REFRAME_ATTEMPTS;
      let thrown: unknown;
      try {
        processDiagnosticBatchSplit(state, wideAggregatorPlan(), task);
      } catch (e) {
        thrown = e;
      }
      expect(thrown).toBeInstanceOf(VerificationTerminalError);
      expect((task as any)._failureReason).toMatch(/단일 세션/);
    });

    it('wide flat plan but AMPLE budget → noop (budget-aware, no false trip)', () => {
      const state = makeState();
      state.recursionLimit = 2000; // est 96 < 1200 → within_budget
      state.recursionCount = 0;
      const task = makeTask({ type: 'feature', priority: 550, name: 'wide but cheap' });
      const plan = wideAggregatorPlan();
      const out = processDiagnosticBatchSplit(state, plan, task);
      expect(out).toBe(plan);
      expect(state.taskQueue.size()).toBe(0);
    ...
    it('concentrated flat plan (≤2 dir buckets) never trips, even with many entries + tight budget', () => {
      const state = makeState();
      state.recursionLimit = 200;
      state.recursionCount = 150; // very tight
      const task = makeTask({ type: 'feature', priority: 550, name: 'uniform card swap' });
      const cards = Array.from({ length: 20 }, (_, i) => ({
        target: `codebase/src/presentation/components/cards/Card${i}.tsx`,
        action: 'swap dark: classes to tokens',
      }));
      const plan = JSON.stringify({ task: { id: 'p', goal: 'token swap' }, implementation: { modify: cards } });
      const out = processDiagnosticBatchSplit(state, plan, task);
      expect(out).toBe(plan);
      expect(state.taskQueue.size()).toBe(0);
    });

    it('error/verification flat plan is NOT subject to the size gate (different shape)', () => {
      const state = makeState();
      state.recursionLimit = 200;
      state.recursionCount = 150;
      // verification task type — not in SIZE_GATE_TYPES.
      const task = makeTask({ type: 'verification', priority: 1000, name: 'final verify' });
      const plan = wideAggregatorPlan();
      const out = processDiagnosticBatchSplit(state, plan, task);
      expect(out).toBe(plan); // noop, no throw
      expect(state.taskQueue.size()).toBe(0);
    });
  });
