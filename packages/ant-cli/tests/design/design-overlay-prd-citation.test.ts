/**
 * Design-Overlay PRD Citation — Invariant Test (Phase D).
 *
 * The plan job authors PRD as the SSOT for product surface (IA /
 * screen composition / content policy / mechanic catalog / entity
 * catalog / etc.). Design jobs MUST cite PRD sections by stable
 ...
 * `PRD §X` / `PRD §X` and the symbolic ID prefixes (SC-, FL-, FR-, CP-,
...
describe('Design overlay — service-axis PRD citation guidance', () => {
  ...
  it('system-design/base.md asks Step 1 Complexity Questions in PRD §X form', () => {
    const src = read('jobs/design/nodes/decompose/variants/system-design/base.md');
    expect(src).toMatch(/PRD §10|PRD §5|PRD §11|PRD §3/);
    // game-side sections still covered by the unified PRD (coreloop /
    // MDA mechanics / content scope) — the document name is now PRD for
    // every domain, but game vocabulary remains.
    expect(src).toMatch(/PRD §2|PRD §4|PRD §8/);
  ...
  it('system-design/base.md adds the Content/UX Level column to Abstraction Level', () => {
    ...
    expect(src).toMatch(/owned by PRD|PRD owns/);
  });

  it('system-design/base.md requires task descriptions to cite the PRD hook', () => {
    const src = read('jobs/design/nodes/decompose/variants/system-design/base.md');
    expect(src).toMatch(/PRD hand-off citation/i);
    expect(src).toMatch(/Architectural — no direct PRD hand-off/);
  });

  it('system-design/rules.md validates the PRD citation in task descriptions', () => {
...
// ──────────────────────────────────────────────────────────────────────
// Game-axis citations (design job consumes PRD)
// ──────────────────────────────────────────────────────────────────────

describe('Design overlay — game-axis PRD citation guidance', () => {
  it('jobs/design/domain/game.md publishes the PRD ↔ design hand-off table', () => {
  ...
  it('game-art-design-by-desc/base.md derives token derivation from PRD §4 Aesthetic / §6', () => {
    const src = read('jobs/design/nodes/decompose/variants/game-art-design-by-desc/base.md');
    expect(src).toMatch(/PRD §4 MDA Aesthetic/);
    expect(src).toMatch(/PRD §6 Reward & Feedback/);
    expect(src).toMatch(/PRD §8 Content Scope/);
  });

  it('game-art-design-by-desc/base.md derives asset categories from PRD §8 EN-/LV-', () => {
    const src = read('jobs/design/nodes/decompose/variants/game-art-design-by-desc/base.md');
  ...
  it('game-art-design-by-desc/base.md derives spec categories from PRD §4 MC- / §6 RW-', () => {
    const src = read('jobs/design/nodes/decompose/variants/game-art-design-by-desc/base.md');
    expect(src).toMatch(/MC-XXX|PRD §4 \/ MC-/);
    expect(src).toMatch(/RW-XXX|PRD §6 \/ RW-/);
  });

  it('game-art-design-by-desc/rules.md validates PRD §X citation per task', () => {
    const src = read('jobs/design/nodes/decompose/variants/game-art-design-by-desc/rules.md');
    expect(src).toMatch(/PRD Hand-off Citation/);
    expect(src).toMatch(/PRD §8|EN-/);
    expect(src).toMatch(/PRD §4|MC-/);
    expect(src).toMatch(/PRD absent — categories extracted from directive/);
  });
  ...
  it('game-art-design-by-figma/base.md aligns EN-XXX with Figma frames', () => {
    ...
    expect(src).toMatch(/PRD §8 Content Scope/);
    expect(src).toMatch(/EN-XXX/);
    expect(src).toMatch(/figma node cited by PRD|frame name match/i);
  });
...
// ──────────────────────────────────────────────────────────────────────
// PRD ↔ Figma conflict policy (F2 follow-up)
//
...
describe('Asset conflict policy — partial + cross-links (F2)', () => {
  ...
  it('asset-conflict-policy partial publishes the visual / behavior / ambiguous axis matrix', () => {
    ...
    expect(src).toMatch(/PRD wins for behavior/);
