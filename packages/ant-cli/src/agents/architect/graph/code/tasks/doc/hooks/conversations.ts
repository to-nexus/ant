/**
 * doc/hooks/conversations.ts — TaskConversationsHook.convKey
 *
 * Per-task conversation scope for doc tasks. Pre-wired for the future
...
export function convKey(task: CodeTask): string {
  return `node:execute:doc:${task.id}`;
}
