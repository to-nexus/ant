/**
 * setup/hooks/conversations.ts — TaskConversationsHook.convKey
 *
 * Per-task conversation scope for setup tasks. Pre-wired for the future
...
export function convKey(task: CodeTask): string {
  return `node:execute:setup:${task.id}`;
}
