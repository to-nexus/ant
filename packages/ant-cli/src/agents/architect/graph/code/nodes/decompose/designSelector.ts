...
export function prepareRacInjection(
  ...
  if (total > artifactBudgetChars) {
    ...
    if (total > artifactBudgetChars) {
      // Every artifact has already been outlined to its floor. Compaction
      // shrinks per-file SIZE, not COUNT, so a genuinely oversized selection can
      // still overshoot the soft budget. We proceed (no silent artifact drop):
      // if the assembled prompt then exceeds the model context window, the LLM
      // call fails with a deterministic "prompt too long" 400, which the runner
      // surfaces as an explicit, non-resumable failure (isPromptTooLongError) —
      // the request must be narrowed rather than silently mutilated.
      console.warn(
