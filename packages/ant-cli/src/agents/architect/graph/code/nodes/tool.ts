...
async function handleRunCommand(
  ...
  if (match) {
    ...
    if (targetPort === ORCHESTRATOR_PORT) {
      const errorMsg = `🚨 BLOCKED: Cannot kill port ${ORCHESTRATOR_PORT} (Ant orchestrator)
...
Port ${ORCHESTRATOR_PORT} is the orchestrator. Any process in /ant/packages/ant-cli/ is orchestrator.`;
