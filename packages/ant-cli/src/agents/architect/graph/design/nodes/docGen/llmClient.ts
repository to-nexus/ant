/**
 * DocGen-phase LLM client resolver for design jobs.
 *
 * Re-resolves the LLM client through the workspace-config-aware factory
 * so the docGen node can pick a different model (e.g. a cheaper tier for
 ...
 * per-node selection stays consistent — before this, docGen was the only
 * design node that ignored its `llmModels.design.docGen` config slot.
 */
...
export async function resolveLLMClient(state: DesignGraphState): Promise<LLMClient | undefined> {
  ...
  return createLLMClient(
    'architect',
    undefined,
    { jobType: 'design', nodeType: 'docGen' },
