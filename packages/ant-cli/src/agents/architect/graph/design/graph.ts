...
async function checkTaskStatus(state: DesignGraphState): Promise<Partial<DesignGraphState>> {
  ...
  // Asset validation gate (WS2 §1b/§1c) — verify external srcs exist AND inline
  // payloads stay under the D21 css-only ceiling before completing ui-assets /
  // game-art-assets tasks. game-art runs the production `validateGameArtAssetCatalog`
  // (D20/D21/I6); ui runs external-src existence. Shared single-owner helper —
  // the parallel worker (`workerCheckTaskStatus`) calls the same functions.
  const taskIsAsset = isAssetTask(state.currentTask?.id);
  if (taskIsAsset && (state._assetValidationRetried || 0) >= 2) {
  ...
  if (taskIsAsset && (state._assetValidationRetried || 0) < 2) {
    ...
    if (!validation.valid) {
      console.warn(`⚠️  [checkTaskStatus] Asset validation failed: ${validation.missingFiles.length} missing/illegal src, ${validation.inlineViolations.length} inline-ceiling violation(s)`);
      for (const f of validation.missingFiles) console.warn(`   - ${f}`);
      for (const v of validation.inlineViolations) console.warn(`   - inline ${v.id}: ${v.reason}`);
