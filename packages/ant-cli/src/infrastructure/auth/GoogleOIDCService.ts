 ...
 * are not fully resolvable (the auth routes answer 503 in that case). Single
 * owner of the env wiring. The redirect_uri must land on the BE host;
 * same-origin deployments fall back to `FRONTEND_URL`.
 */
export function createGoogleOidcServiceFromEnv(): GoogleOIDCService | undefined {
