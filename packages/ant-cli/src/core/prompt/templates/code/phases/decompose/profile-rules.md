...
3. PRD describes the overall project and may cover a broader scope than the current workspace.
   Do NOT infer environment from PRD alone when design documents are present.

**Constraint — MANDATORY CHECK before setting environment:**
...
2. If tier-prefixed documents (`fe-` or `be-`) exist, the profile language and framework
...
3. If ONLY `system-design (unified)` exists with no tier-prefixed documents,
   determine the profile from the document content, directive, and PRD.

**Constraint**: If the Design Document Availability shows ONLY documents with `be-` prefix,
the profile MUST reflect the backend's language and framework.
...
the profile MUST reflect the frontend's language and framework.
...
to the current profile. The same applies in reverse.
