...
#### 6.2: Scope Breadth + Source Adequacy (modification intent only)

**Principle (RAC-aware)**: When the directive spans multiple boundaries AND the user has not explicitly pinned reference documents for this turn, the work needs a spec/design pass before code can land confidently. Workspace-wide document presence is irrelevant here — only the user's explicit injection counts as source.
...
1. **Scope breadth** — observe the specificity of the directive's target:
   ...
   - **Multi-boundary**: everything else — multiple concerns simultaneously addressed (e.g., UI policy + runtime fix, FE + BE, multiple features), broad/vague directive, cross-layer impact, empty project.
   - **Constraint**: When uncertain, default to multi-boundary.

2. **Source adequacy** — observe `## USER-PINNED REFERENCES` in WORKSPACE STATE:
   - **Pinned present**: count > 0 (user explicitly attached `@`-mentioned refs / context for this turn)
   - **Pinned absent**: count == 0 (no user-pinned refs, regardless of which docs exist in the workspace at large)
   - **Constraint**: Auto-injected role-based artifacts (system design docs, etc.) do NOT count as user-pinned. Only the explicit per-turn `actionMetadata.refs` / `actionMetadata.context` populated by the user counts.

**Decision**:

| Scope breadth | Source adequacy | Target |
|---------------|-----------------|--------|
| Single-boundary | Any | `code` |
| Multi-boundary | Pinned present | `code` |
| Multi-boundary | Pinned absent | `design` (suggest spec design before code) |
...
⚠️ **Blind spot — auto-injected ≠ user-pinned**: Workspace may contain many design/spec documents that get role-based auto-injected later in the pipeline. Those do NOT signal "the user supplied source for THIS directive". Only `## USER-PINNED REFERENCES` counts here.
...
⚠️ **Multi-boundary modification requires user-pinned source**: When the directive spans multiple boundaries AND the user has not pinned any reference for this turn, redirect to design (spec mode) before attempting code. Auto-injected workspace docs do NOT substitute for user-pinned references. (Step 6.2)
