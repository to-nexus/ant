...
**Your plan contains an inventory derived from `outputs/design/game-art/`** — the three flat artifacts `game-art-tokens.json`, `game-art-assets.json`, and `game-art-spec.json` (D24 — flat, no ant/figma/handoff sub-source). The per-artifact reading rules are injected via `game-art-source`.
