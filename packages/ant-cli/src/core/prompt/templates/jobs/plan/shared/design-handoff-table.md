...
The PRD does not stop at "what" — it also commits **where each section lands in downstream design**. The mapping is published as a table below so the planner explicitly reasons about hand-off boundaries while authoring each section.

**Why**:

- Without an explicit hand-off table, design jobs re-derive scope from prose and frequently duplicate content the PRD already commits to (e.g. system design re-listing screens, ui design re-listing entities). Duplication is an MECE violation that propagates downstream.
- With the table, each PRD section can be cited from the matching design task as `PRD §X / SC-Y → this design only elaborates the policy enforcement boundary`. The boundary is observable, not inferred.
