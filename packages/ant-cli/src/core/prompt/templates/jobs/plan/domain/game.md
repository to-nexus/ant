...
| 9 | Input & Perspective | Control & camera contract | Control scheme, render dimension + viewpoint (2D side / 2D top / 2D iso / 3D — the 2D-vs-3D choice is what downstream design / code adopt as the `perspective` axis), device target. **Both `orientation policy` (locked-portrait / locked-landscape / fluid) AND `viewport target` (fullscreen / windowed) are committed on a single explicit line — omitting either lets downstream design / code default silently.** |
