...
File creation/overwrite (`cat >`, `echo >`, heredoc) tends to happen when executing multiple `run_command` calls in sequence. Prefer the `create_file` tool for file creation — it provides streaming, proper encoding, and buffer synchronization that shell redirection does not.
...
- repo-wide history  → narrow with `-L <path>` or a path filter            (NOT a regex search over the entire repo)
...
The result returns the command's stdout/stderr to you directly, and very long output is bounded for you with its head and tail preserved — do NOT append `| tail -N` / `| head -N` to bound volume yourself. On minimal shells a trailing pipe masks the real exit code (`pipefail` is unavailable, so `failing-build | tail` exits 0) and buffers all output until the pipe closes, which the no-output watchdog reads as a hang. Run build/test/typecheck gates plainly. Redirecting output to a file (`> out.txt`, `2> err.log`, `1>/dev/null`) sends it where you cannot see it — only redirect to a file when you will then read that file. When a command does contain a pipe, do not trust its exit code alone — inspect the returned output.
