...
────────────────────────────────────────────────────────────────────────────────
## Tool Observation Categories
────────────────────────────────────────────────────────────────────────────────

### Category A — Design-Prescribed Dependency Discovery
...
**Principle**: Observe the actual exported API signatures of each prescribed dependency before writing the plan. Record them inline in the `purpose`/`changes` of the `implementation.modify`/`create` entry that uses the package so execute can locate the right source to verify against — execute still cross-checks the exact shape at write-time, but it benefits from your sketch as a pointer.

**Constraint**: Record observed signatures with parameter and return types when reasonable. Sketch-level natural-language form is acceptable when full type capture would be noisy; execute verifies the precise shape against the package source.
...
5. Record the import path + observed signatures inline in the `purpose` (for `create`) or `changes` (for `modify`) of the entry that uses the package. This serves as a sketch for the implementation phase — execute verifies the exact shape against the package source at write-time, so partial or natural-language signatures are sufficient when full type capture would be noisy. If several entries share the same package, declare the signatures in the first entry and reference by name in the rest.
...
**Constraint**: Every design-prescribed dependency whose API was observed via tools SHOULD appear inline in the entry that uses it. A discovered signature omitted from `implementation.*` entries forces execute to re-discover it — inline the sketch to avoid duplicate observation cost. Execute still verifies the exact shape against the package source.

### Category B — Codebase Observation
...
### Out of scope — Well-Known Packages

(This is a constraint axis, not a category — tools should NOT be spent here regardless of the categories above.)
