...
- **A navigation target is one of these hooks, and its producer is a route that must EXIST.** Emitting a link / programmatic navigation / redirect target is valid only when it equals a path that some task actually authors in the route tree (the addressing scheme's producer — see the Shared Decisions principle). A target pointing at a path no route serves is a dead link: it builds green and fails at runtime — a 404, or a silent fall-through to a different dynamic segment that happens to match. Key an entity route by the entity's stable id, never by a human-readable name. If the path you need has no owning route, that is a missing route to surface as the route owner's gap, not a target to emit against a route that does not exist.
