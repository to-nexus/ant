...
- The description under each task is TRUNCATED. When you need a prior task's full scope/intent — chiefly the feature task whose surface your task pairs with or extends — call `read_state` with that task's name to expand it to its full description and complete file manifest.
