...
Path: `outputs/design/game-art/ant/` (sub-sourced canonical, mirrors `outputs/design/ui/ant/`). The `figma/` and `handoff/` sub-sources are Phase 5+ hooks (parser-only today). Sections of `game-art-spec.json` are addressable by category key; the pool exposes each as `outputs/design/game-art/ant/spec/{category}`.
