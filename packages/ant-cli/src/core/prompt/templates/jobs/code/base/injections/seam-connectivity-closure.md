...
denominator.** Walk it file by file: for EACH file, account for every reference it
emits (navigation/call/event/dependency/import/style-selector) and every
interactive control it renders, and record each as resolved, to-fix, or to-remove.
A file left unexamined is a hole in the closure. Where parts diverge on a shared
