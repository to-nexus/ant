...
**Principle**: The base path is applied in exactly ONE place per URL, and that place is decided by whether the URL travels THROUGH the framework's own routing or is handed OUTSIDE it — NOT by whether you happened to assemble the string by hand.

- **Through a framework navigation primitive** — the router's `navigate` / `push` / `replace`, a declarative link the framework renders (`<Link href>` and the anchors it emits), or a framework-resolved asset reference. The framework auto-prefixes the base path ONLY for these. Do **NOT** add the prefix yourself — the framework already did, and a second prefix applies it twice.
- **Handed outside the framework's routing** — a URL string you build and pass to something other than the framework router. This does NOT pass through the framework's prefixing, so it MUST include the base-path prefix explicitly, read from the same environment variable the framework config uses (the variable in the Contract table above for the active framework).

**Constraint**: The "handed outside" case covers — but is not limited to — a redirect / callback target (an OAuth `redirect_uri`, an SSO return URL) handed to an external authority; a full-page browser navigation you perform yourself (`window.location`, `location.assign`, `location.href`) that bypasses the framework router; and a declarative link or asset reference whose target you write as a literal absolute path (a markup anchor `href` / image `src` set to `/<path>` rather than produced by the framework's navigation primitive — the framework prefixes only its own primitive, not a literal you typed). For each, build it as `origin + <basePath> + "/<path>"`, reading `<basePath>` from the framework's base-path variable in the Contract table. Concatenating the runtime origin with a bare path (omitting the prefix) yields a URL that resolves only when the app is served at the root and breaks the instant it is served under a path prefix — which every Ant-managed preview is.

⚠️ **Blind spot (under-prefixing)**: The framework prefixing its own links creates the false impression that ALL URLs are prefixed. The escape hatch is exactly the hand-built absolute URL handed outside the router — most commonly an authority redirect / callback URI assembled from `origin + "/<path>"`.

⚠️ **Blind spot (over-prefixing)**: The inverse error breaks just as hard. Prepending the base-path variable to an argument of the framework router (`navigate` / `push` / `replace`) or to a `<Link href>` double-applies the prefix — the framework adds it again, producing `/<base>/<base>/<path>`. A path handed to a framework navigation primitive MUST stay bare; applying the prefix there is the framework's job, never yours.
...
| Consumer runtime | Toggle name | Example (`@connection business stripe-api`) |
|---|---|---|
| Server-only (Node, RSC, server actions, backend services) | `USE_MOCK_<NAME>` | `USE_MOCK_STRIPE_API` |
| Next.js — adapter selected from a client component or a module imported by one | `NEXT_PUBLIC_USE_MOCK_<NAME>` | `NEXT_PUBLIC_USE_MOCK_STRIPE_API` |
| Vite (React / Vue SPA) | `VITE_USE_MOCK_<NAME>` | `VITE_USE_MOCK_STRIPE_API` |
| CRA | `REACT_APP_USE_MOCK_<NAME>` | `REACT_APP_USE_MOCK_STRIPE_API` |

`<NAME>` = uppercase snake of the `@connection` name (hyphens → underscores). Author exactly this one per-connection toggle. A master broadcast (`<PREFIX>USE_MOCK`) is NOT part of the generated output and the selection code does NOT read it — the runtime honours it only as a fallback if a human adds one.
...
### One Connection, One Toggle

The toggle is keyed on the `@connection` — NOT on the adapter, the consuming feature, or the resource concern. When several adapters virtualize the SAME backend connection (e.g. a data-access adapter and an auth adapter both reaching one backend host through one URL variable), they MUST ALL read the single connection-derived toggle `<PREFIX>USE_MOCK_<NAME>`. Do NOT mint a separate toggle per concern.

**Constraint**: A toggle name that is not the uppercase-snake of an existing `@connection` name is a defect — it is unreachable from the preview connection switch, so flipping that service to its real adapter writes the derived name while the app keeps reading the off-contract name, and the virtualized adapter silently stays active.

When two concerns are genuinely separate external services with distinct endpoints (distinct URL variables), declare them as two `@connection`s — each then owns its own derived toggle and its own preview switch. Sharing one endpoint means one connection and one toggle; distinct endpoints mean distinct connections.

⚠️ **Blind Spot**: concern-named sub-toggles (`USE_MOCK_API`, `USE_MOCK_AUTH`) are EASILY invented when one backend serves multiple adapters. They are NOT connection names, so the preview switch cannot reach them. Collapse them onto the one connection-derived toggle.
...
- Exactly one toggle per `@connection`, named `<PREFIX>USE_MOCK_<NAME>` from the connection name. Do NOT author a master broadcast (`<PREFIX>USE_MOCK`) and do NOT mint concern-named sub-toggles (`USE_MOCK_API` / `USE_MOCK_AUTH`) for adapters that share one backend connection — both are unreachable from the preview connection switch.
