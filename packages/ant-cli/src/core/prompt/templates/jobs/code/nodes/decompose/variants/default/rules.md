...
**Constraint**: Work-style keywords in the directive (`generate` / `refactor` / `explain`, or their equivalents in the user's language) are **framing signals**, NOT `type` values. Choose `type` from the schema enum by what the task DOES (broken behavior → `"error"`; new behavior → `"feature"`; visual → `"ui"`; etc.) — never copy a mode keyword into `type`. The only mode name that is also a valid `type` is `"explain"`.
...
- Directory-per-surface pattern (e.g. `screens/login/`, `pages/dashboard/`, or `scenes/level-1/`) → one ui task per surface directory.
...
- **A shared library is justified by shared *consumption*, not by a dedicated document**: a body of code that ≥ 2 applications consume identically is a shared-library package even when no specification document names it. Name it by its **purpose**, not by a document. ⚠️ The most easily-missed shared library is a visual foundation / component library that multiple frontend applications share — it rarely has its own document yet every frontend consumes it; do NOT bury it inside one application. Conversely, code that merely *implements against* a shared contract (clients, business logic, infra adapters) is NOT shared — it lives in each consuming application's own layer.
- Monorepo (multiple units) -> one `band:'root'` setup at **priority 100** (workspace-level only: root manifest + member glob + shared config + infra) + one band-absent setup per member at **priorities 101, 102, …** (each owns its own member directory + manifest/name + skeleton). The priority-100 root setup is unique and runs first.
- Monolithic (single application, no shared library) -> one `band:'root'` setup at **priority 100** (the lone package manifest + framework/tooling config) + one band-absent setup at **101** (the app's `src/` skeleton only). Same band shape as a monorepo with one member — uniform.
- **Sibling-app topology consistency**: When the spec yields 2+ frontend application packages on the SAME stack, their per-package setup tasks MUST seal the SAME 1st-order composition axis (feature-primary `<feature>/<layer>` vs layer-primary `<layer>/<feature>`). Read each fe-system doc's §Directory Structure & Boundary Mapping (and §Overview composition line): if they agree, bind all sibling setups to that axis; if a doc is silent, default all siblings to the axis a specifying doc states; diverge only when a doc explicitly justifies it. Do NOT let two sibling setup tasks independently pick opposite axes.
- **Member identity is bound, not re-invented (every task, not just setup)**: A workspace member's identity — its directory name AND its `package.json` name/scope — has one authority: the name already present in the workspace (sealed by setup, or pre-existing in an existing codebase). When referencing or extending a member that ALREADY exists, observe its EXACT name via `list_files` / the workspace members summary and use it verbatim — never a re-derived, pluralized, re-scoped, or synonym variant (this is the cross-task extension of the setup directory-sealing "no renamed sibling" rule; calling one boundary by different names across tasks strands orphaned, unreconciled packages). When a task genuinely needs a member that does NOT yet exist, FIRST `list_files` to check whether an existing member already serves that purpose: if one does, bind to it; only if none does may the task create a new, purpose-named member (no setup task is required — a member may be added mid-project, e.g. in an existing-codebase job that has no setup task). Adding a genuinely new, distinct member is allowed; duplicating, renaming, or near-missing an existing boundary is not.
- **Setup ownership is split by scheduling band — `band:'root'` vs band-absent**: A setup task's job is fixed by an explicit band (Three-Axis SSOT), derived from its priority. **`band:'root'`** = the project/framework/workspace-level setup, emitted at **`priority` 100 (SETUP_PROJECT), exactly one per job** (lowest priority → runs first). It owns ONLY root-level artifacts — in a monorepo: the root manifest + the workspace package glob (e.g. `apps/*` / `packages/*`) + the root tsconfig base + `.gitignore` + workspace infra (docker-compose / root `.env*`); in a monolith: the lone package manifest + framework/tooling config. **A `band:'root'` setup creates NO member/package directory, no member manifest, and no member name.** **Band-absent setup tasks** (priority 101+) each own exactly ONE member fully — its own directory, its own manifest (whose `name` is that member's identity SSOT, derived from purpose), and its own internal source skeleton; in a monolith the single band-absent setup owns only the `src/` skeleton (the lone manifest belongs to the `band:'root'` setup). **No setup creates a directory or names a member it does not own.** The workspace glob discovers members, so the root setup never enumerates them — which is why two setups can never strand a losing naming scheme (`contract` vs `contract-types`, `design-system` vs `ui`) as orphan directories. Observable: exactly one `band:'root'` / priority-100 setup; exactly one band-absent setup per member; root setup touches zero member paths.
...
- Initial directory skeleton — the setup that OWNS a unit seals that unit's top-level source directory tree (with `.gitkeep`) reflecting architecture boundaries from the system design (or framework convention if no system design). Sibling and future tasks bind to this skeleton via `list_files` as the structural context. In a multi-package workspace, **each member's own band-absent setup creates and names that member's directory** (the `band:'root'` setup creates none); a content task populates the directory its member setup created and MUST NOT create a renamed sibling for the same boundary. In a monolith the single setup seals the app's source tree directly.
...
- a **sign-in authorize / callback address every auth adapter and login surface in one runtime must agree on** — the URL the browser is sent to in order to begin sign-in, and the address it returns to, is a shared decision: when more than one adapter or surface in a runtime constructs that address independently, they diverge silently (one resolves in-app, another points at a fabricated external or fixed-origin host) and which one fails is non-deterministic. The runtime's auth-flow owner (the same `platform`-band producer that owns the sign-in entry above) owns ONE authorize/callback route constant for that runtime; every auth adapter and login surface in the runtime CONSUMES it. No adapter or surface authors that authorize/callback address as a string literal — it references the owned constant. This is a per-runtime address-FORM decision, NOT a cross-runtime boundary: it lives alongside the runtime's own session boundary (each buildable app owns its own constant), so converging the form does not collapse the per-runtime session isolation. A monorepo with N buildable apps therefore has N such producers — one per runtime — never a single auth boundary shared across apps, and never N adapters each inventing the address.
