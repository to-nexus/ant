...
- **Member identity is bound, not re-invented (every task, not just setup)**: A workspace member's identity — its directory name AND its `package.json` name/scope — is sealed once (root setup seals the member directories; a per-package setup fixes the manifest name). EVERY task that references another member — an import path, a dependency entry (`@scope/name` / `workspace:*`), or a target directory — MUST use the EXACT name the workspace already shows (observe via `list_files` / the workspace members summary), never a re-derived, pluralized, re-scoped, or synonym variant. A task that needs a member which is not yet sealed does NOT invent one; the missing member is a decomposition gap (add the sealing setup), not a name to improvise. This is the cross-task extension of the setup directory-sealing "no renamed sibling" rule — naming one boundary differently across separate tasks strands orphaned, unreconciled packages.
