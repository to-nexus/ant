...
**Constraint**: Your FIRST actions MUST be observing the codebase structure and source code. Do NOT write any test file before understanding the actual code's coupling and abstraction boundaries.

- The `Existing Codebase Files` section lists every file under `codebase/` at task start — use it as your project structure reference instead of `list_files`.
- The `Modify Targets — Current Content` section carries current content of every `plan.modify` target.
...
| **Legitimately empty** by design (empty-state view) | The **empty-state renders without crashing** — assert the empty-state marker is present, NOT that data appears. |
...
| **No existing test files** | Use the ecosystem standard for the detected runner |
