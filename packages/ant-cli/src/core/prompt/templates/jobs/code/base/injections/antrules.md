...
- **Shared-contract authority pointers** — when one module/package is the single source for a family of symbols every consumer must bind to, record the *pointer + binding discipline*, NOT the symbols' shapes: "request/response & enum types import from `@scope/contract` only — never redefine locally", "the API port interface lives at `path/to/port` — bind to it, do not reconstruct", "shared-UI identifier vocabulary is camelCase (e.g. `chevronLeft`, not `chevron-left`)", "styling is a project-authored utility-class set (`row`/`stack`/…), not a generated-utility framework — markup references only selector names the styling source defines". This anchors consumers to read the authority instead of guessing its surface; the surface itself (members/fields) stays in the defining file (condition 2).
- **Shared-service consumption / dependency-injection convention** — *how* consumers obtain a shared runtime service (e.g. injected via an app context/provider vs imported as a module singleton). This is a wiring PATTERN every consumer repeats, not a single symbol's shape — so it passes condition 2 the way naming conventions do. Recording it once keeps parallel app/package tasks from diverging (one app injecting via context while a sibling app imports the singleton directly). Record the chosen mechanism, not the service's API (the example is one stack's instance — adapt to yours; e.g. a frontend app: "consumers obtain the shared port via context injection — never import the adapter singleton directly"). Especially worth recording in a monorepo where multiple apps must stay consistent.
...
Typical legitimate seeds: project-specific naming/structure conventions not encoded in tool configs (these recur across siblings, so they pass condition 2); a **shared-service consumption / DI convention** (*how* consumers obtain a shared runtime service — e.g. "hooks get the API port via context injection, never import the adapter singleton"; a wiring PATTERN that keeps parallel app/package tasks consistent, especially in a monorepo); or a point-in-time package pinning rationale (e.g. "`shadcn X v0.4` breaks with `react@19` — pinned to 18"). Do NOT seed framework / library / alias / source-root restatements — those live in `package.json` / `tsconfig.json` already and duplicating them creates drift.
