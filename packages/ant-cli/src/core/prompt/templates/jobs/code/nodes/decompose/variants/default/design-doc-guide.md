...
**Constraint — every shared-library package is a sealed member**: When a shared library is extracted (a shared design system / component library, shared contract types, shared domain), it is one of the "multiple units" above — the root setup MUST enumerate it as a sealed member directory (the binding name) AND it gets its own per-package setup, exactly like the shared contract library. This is true regardless of which task type later fills it: a shared design system that ≥ 2 frontend apps consume is sealed here as a package member, so its name is fixed once at root setup. Do NOT leave a shared library's package to be named ad-hoc by a later content task — when separate tasks each name the same boundary independently, they strand divergent, orphaned package directories.
