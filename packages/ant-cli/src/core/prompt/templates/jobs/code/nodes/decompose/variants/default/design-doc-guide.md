...
**Constraint**: A complete application document (`*-system-{name}.md`) usually corresponds to one application package, whose directory leaf is the application's **own name/identity** (its purpose) — not the document's filename token, and never an `fe-`/`be-` tier prefix. But a document is a design source, not a package blueprint: its content may also seed shared-library packages, and a shared-library package is named by its **purpose** — it need not mirror any document name. `api-contract-*` is a frontend↔backend wire-protocol contract — it documents an *interface*, not an application, so it never becomes an application package, and its presence does NOT by itself imply a backend application is part of this job (its consumer may be frontend-only, e.g. mock adapters). Only the shared *contract types* are extracted to a shared-library package (and only when ≥ 2 units consume them); the code that *implements* the contract lives in each consumer's own layer, not in the shared package.
...
**Setup task mapping** (uniform band shape — always one `band:'root'` + one band-absent per member):
- **One application, no shared library** → `band:'root'` setup (priority 100, exclusive — the lone manifest + framework/tooling config) + one band-absent setup (priority 101 — the app's `src/` skeleton only).
- **Multiple units (applications and/or shared libraries)** → `band:'root'` setup (priority 100, exclusive — root manifest + member glob + shared config + infra; creates no member directory) + one band-absent setup per member (priority 101+, non-exclusive, distinct parallelGroup per package — each creates and names its own member). The physical workspace layout (deployable apps vs shared libraries) is the `band:'root'` setup's concern; member identity is each member setup's concern.

**Constraint — every shared-library package gets its own band-absent setup**: When a shared library is extracted (a shared design system / component library, shared contract types, shared domain), it is one of the "multiple units" above — it gets **exactly one band-absent setup task (priority 101+) that creates and names it**, exactly like every other member. The `band:'root'` setup (priority 100) does NOT enumerate or name it — the workspace glob discovers it. Its `package.json` `name` is fixed once, by its own setup, derived from purpose. This holds regardless of which task type later fills it: a shared design system that ≥ 2 frontend apps consume gets its own setup here. Do NOT name a shared library's package from a later content task, and do NOT let two tasks target the same boundary — exactly one setup owns each member, so no divergent, orphaned package directories can be stranded.
