...
- `kind: 'external'` — `src` (`assets/game/...`) names the source-of-truth file; it is NOT assumed to be a directly-fetchable URL. The file is made web-servable per the framework's static-asset convention (the code job's asset-placement guidance commits the concrete destination), and the engine loader registers it under `entry.id` verbatim (the key the spec references). The loading boundary depends on the engine partial's preload API; Domain does NOT load assets directly. See `jobs/code/basis/gameArtTier/_preamble.md` §3.4 for the path-convention SSOT.
