...
- Coverage enumeration lives in the WORK tasks' descriptions. The verification task's `description` names its gate set (its `include` ref makes the acceptance criteria visible to the verification pass) — it does NOT re-enumerate the document's units.
