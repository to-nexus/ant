...
- **The authorize / account-selection entry is served in-app.** The URL the browser is navigated to in order to begin or confirm sign-in MUST resolve to a surface the running app itself serves at its own runtime origin — never an external or unreachable host literal (e.g. a placeholder `*.example.com`). This is the outbound sibling of the callback rule below: a hardcoded external authorize host dead-ends the flow exactly as a hardcoded callback host does, because the closed system cannot answer it.
