...
- `<Image src="https://...">` without the host in `next.config.*` `images.remotePatterns` → client-hydration throw "hostname not configured"; build passes (validation is browser-only). Component and `next.config.*` MUST update together. Separately, the optimizer fetches the URL server-side and decides validity by content-sniffing the response body — environment-dependent failures (proxy serving HTML for the CDN, restricted egress on the redirect destination, stale negative cache) surface as 400 "not a valid image" even when the listed host IS allowlisted. For service-virtualization placeholders, bypass the optimizer entirely (`unoptimized` or plain `<img>`, per `service-virtualization-imagery` pathway 3) rather than enumerating hosts in `remotePatterns`.
