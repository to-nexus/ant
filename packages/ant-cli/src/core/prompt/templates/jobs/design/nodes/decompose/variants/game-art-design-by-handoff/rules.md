...
Refactor mode: the path is not invented — it is one of the manifest paths from the input context with the `visual/game-art/handoff/` prefix stripped (bundle-relative), or a `"newFile": true` addition inside a directory the bundle already has. `read_file`, by contrast, takes the full manifest path as shown.
...
| 2 — shared layers | shared component css / entity css / asset files | 200–249 |
| 3 — consumers | screen files, component specimen pages, entity specimen pages | 300–349 |

A specimen page composes the class names its `components/<name>.css` or `entities/<name>.css` declares, so it is a CONSUMER of that file, not its peer — it never shares a stage with it. Two tasks in the same stage run at the same instant and cannot read each other's output.
...
- Every stage-2/3 task description names the exact bundle-relative files it must read before authoring: its token concern files; for a screen, the component/entity css it composes; for a specimen, its own `components/<name>.css` or `entities/<name>.css`. A consumer that is not told what to read invents names.
...
- A named widget or entity → its own `<surface>/<name>` pair: `.css` at stage 2, specimen `.html` at stage 3.
...
⚠️ **Blind spot**: reading "component" as "named widget" leaves the entire element-level layer ownerless — the widgets bind and the pages around them do not.

### 7. Source File Assignment
...
<tasks>
  ...
  <task>{"id":"game-art-handoff-component-primitives-css","name":"Primitives: element-level roles","targetFile":"components/primitives.css"{{#if sourceFileNames}},"sourceFiles":["<source filename>"]{{/if}},"description":"Own the element-level classes every page composes (text/heading roles, buttons, page shell, loading placeholders); read tokens/<concern>.css first and build only on its variables.","priority":200,"parallelGroup":"game-art-handoff-component-primitives-css"}</task>
  <task>{"id":"game-art-handoff-entity-player-html","name":"Specimen: Player entity","targetFile":"entities/player.html"{{#if sourceFileNames}},"sourceFiles":["<source filename>"]{{/if}},"description":"Specimen page for every player-entity state; read entities/player.css first and compose only the classes it declares; demo-page scaffolding stays in a local <style>.","priority":300,"parallelGroup":"game-art-handoff-entity-player-html"}</task>
  <task>{"id":"game-art-handoff-screen-hud","name":"Screen: In-Game HUD","targetFile":"screens/hud.html"{{#if sourceFileNames}},"sourceFiles":["<source filename>"]{{/if}},"description":"Realize the HUD per DESIGN.md and PRD; link ../styles.css; read the component css it composes; screen-local layout only.","priority":300,"parallelGroup":"game-art-handoff-screen-hud"}</task>
</tasks>
```
...
- ✅ Priorities respect the stage table (tokens/guide < component/entity css and assets < screens and specimens)
- ✅ Every `components/*.html` / `entities/*.html` specimen task sits at stage 3, after its `.css`
