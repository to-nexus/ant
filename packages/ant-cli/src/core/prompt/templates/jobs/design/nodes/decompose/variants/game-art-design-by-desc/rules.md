...
### 7.5 PRD Hand-off Citation (when PRD is in source documents)
...
- Each `game-art-assets-{category}` task description MUST cite the
  ...
  (e.g., `Implements PRD §8 (EN-Hero, EN-Hero-Wounded)`)
- Each `game-art-spec-{category}` task description MUST cite the
  `MC-` / `RW-` IDs (e.g., `Implements PRD §4 / MC-Combat`)
- The `game-art-tokens` task description MUST cite `PRD §4 / §6`
  Aesthetic + Reward & Feedback vocabulary
- An asset / spec task without a PRD citation creates shadow IDs that
  diverge from the planner's commitment. When the PRD is genuinely
  absent, state `PRD absent — categories extracted from directive` in
...
- ✅ External entries only present if the directive explicitly references
  a user-placed file (or the PRD §8 cites a sprite path for that entity)

### PRD Hand-off Citation
- ✅ When PRD is in source documents: every `game-art-assets-{category}`
  description cites the `EN-` / `LV-` IDs it covers
- ✅ When PRD is in source documents: every `game-art-spec-{category}`
  description cites the `MC-` / `RW-` IDs it covers
- ✅ When PRD is in source documents: `game-art-tokens` description
  cites `PRD §4 / §6` Aesthetic + Reward vocabulary
- ✅ When PRD is absent: every task description states
  `PRD absent — categories extracted from directive`
