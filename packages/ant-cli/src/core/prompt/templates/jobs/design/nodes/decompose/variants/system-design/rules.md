...
| Re-listing PRD identifiers without citation | Citing the PRD section + symbolic ID this task elaborates (e.g., `Implements PRD §10 / RB-Seller`, `Implements PRD §4 / MC-Combat`). Architectural tasks with no direct hand-off MUST state `Architectural — no direct PRD hand-off`. |
...
**Constraint — sibling FE topology consistency**: When `fePackages` has 2+ entries on the same stack, every `fe-system-{package}` task description MUST carry the directive: *if feature grouping + explicit layers are chosen (frontend-guide §1.2 Dimension 3), use a composition topology (feature-primary vs layer-primary) consistent with sibling FE packages of the same stack, unless this package's stated complexity difference justifies divergence.* (These execute tasks run in parallel and cannot read each other, so this anchors them to the same default; the coding phase enforces the final consistency.)
...
- ✅ Description either cites the PRD §X / symbolic ID this task elaborates, or explicitly marks itself "Architectural — no direct PRD hand-off"
