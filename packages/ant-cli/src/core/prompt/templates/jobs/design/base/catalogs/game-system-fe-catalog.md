### § Overview
- System purpose and the architecture decision per dimension with rationale (reference the observed genre / coreloop from the GDD)
- Which boundary owns the authoritative simulation vs. which only renders it — state the separation up front
- GDD constraints relevant to architecture (viewport / orientation, input scheme, single-screen vs. camera-panned) — cite the GDD section, do not restate content
...
- The entity catalog the simulation advances — cite GDD `EN-XXX` identifiers; do NOT re-derive the catalog
...
- The valid phases and allowed transitions (e.g. a coreloop's ready → active → resolved cycle) — cite the GDD coreloop (`CL-XXX`) the machine realizes
...
- Fail / restart transitions and their cost — cite the GDD fail condition
...
- Name the key contracts/boundaries involved (sync strategy, command channel, state provider) and their roles — cite the GDD game mode (`GM-XXX`)
