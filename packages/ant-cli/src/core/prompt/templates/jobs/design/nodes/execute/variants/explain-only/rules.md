...
**Why it matters.** A reply that calls a file-writing tool or names a target filename causes the design job to write a document the user never asked for. The user requested an explanation; honour that.

**How to apply.**

- Do NOT call file-writing tools (`create_file` / `append_file` / `edit_file` / `delete_file`).
