...
This preamble is injected for game-art **design** intents (`gen-art-figma` /
`gen-art-desc` / `rev-art`). It defines the responsibility boundary of
art design — what kinds of assets the LLM is allowed to author inline
...
### 5. Invariants (recap — I6 / I7)

- **I6 — Asset Surface Boundary**: `kind: 'external'.src` MUST start
  with `inputs/assets/game/`. Never reach into `inputs/assets/service/`
  from `game-art-assets.json` (and never reach into `inputs/assets/game/`
  from `ui-assets.json`).
- **I7 — Art Design Surface**: art-design prompts / outputs MUST NOT
  cross-pollinate with `visualLanguage` / `surfaceSystem` / `spatialSystem`
  (those belong to `visualTier` and live on the UI surface).
...
which means an `intentGroup === 'design-art'` intent (i.e. `gen-art-figma`
/ `gen-art-desc` / `rev-art`). UI / system / spec design intents
...
- Categorize by asset type (`entities`, `particles`, `projectiles`,
  `sfx`, `bgm`, `tilemaps`, ...). Category keys are LLM-chosen but
  must remain stable inside one design pass — `game-art-spec.json`
  references entries by `id`, so renaming after the fact corrupts the
  spec doc.
...
### D. Cross-Surface Boundary (I7 — Art Design Surface)

Game-art design MUST stay on its own surface and MUST NOT cross-pollinate
with the UI surface:

- ❌ Do NOT write `visualLanguage` / `surfaceSystem` / `spatialSystem`
  decisions inside `game-art-tokens.json`. Those belong to
  `outputs/design/ui/ant/ui-tokens.json` (visualTier / UI design intent).
- ❌ Do NOT reference `inputs/assets/service/...` from
  `game-art-assets.json` (I6 — Asset Surface Boundary).
- ✅ DO reference the same PRD and system-design RAC pool — both
  surfaces consume the same upstream signal.
