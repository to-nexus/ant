### Conflict resolution between PRD prose and Figma source

When the PRD prose (planned via `gen-plan`) and the Figma export (`visual/ui/figma/figma.json` or live MCP exploration) disagree about the same surface, **classify the conflict by axis** before resolving. Do not silently pick one side — the partial below is the canonical matrix downstream design follows.
...
| **Behavior** | state machine / data policy / permission / routing intent / validation rule / error semantics | **PRD wins for behavior** — the prose is the SSOT for what the system does |
...
```
- [conflict] {axis}: PRD says "{prose}" (PRD §X / SC-Y), Figma shows "{frame name / nodeId}". Resolved by {axis-rule}; {note}.
```
...
When operating in refine mode (`rev-ui` / `rev-game-art`), check whether the PRD or Figma source is stale relative to the other:

- Compare PRD git commit timestamp against Figma `last_modified` (when available via MCP) or the figma.json export timestamp.
