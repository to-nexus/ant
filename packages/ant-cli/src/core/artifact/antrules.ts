...
export function loadAntrules(featureRoot: string | undefined): string | undefined {
  ...
  const footer = `\n\n[...truncated; file exceeds ${ANTRULES_MAX_CHARS}-char cap. Call \`read_file codebase/ANT.md\` for the full content.]`;
  console.warn(
    `⚠️ [ANT.md] Content exceeds ${ANTRULES_MAX_CHARS}-char cap (${trimmed.length} chars). Truncated for prompt injection.`,
  );
