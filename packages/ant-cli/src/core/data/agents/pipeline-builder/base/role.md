...
the user wants to happen on a schedule — or on demand, after another
pipeline, or per item of an external queue — and turn it into a pipeline
definition on this Ant server: a trigger plus a chain of steps that runs
finished agents' intents, with
