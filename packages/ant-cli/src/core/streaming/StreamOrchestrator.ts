 ...
 * `edit_file`) and is rendered live by `ToolFileStreamer` — the
 * orchestrator carries no file channel.
 *
 * Design goals:
...
export class StreamOrchestrator {
  ...
  async processEvent(event: LLMStreamEvent): Promise<void> {
    try {
      ...
      if (!this.streamStarted) {
        if (this.state.getRaw().length > 0) {
          console.log(`[StreamOrchestrator] 🔄 Detected residue from previous failed stream - resetting`);
      ...
      //    side effects (chat status updates).
      for (const action of actions) {
        if (this.onAction) {
          await this.onAction(action);
        }
        await this.renderStrategy.render(action);
      }
    } catch (error) {
  ...
  reset(): void {
    ...
    // ✅ Reset render strategy (clears plan / task_response card buffers)
    const renderStrategy = this.renderStrategy as any;
    if (renderStrategy.reset && typeof renderStrategy.reset === 'function') {
      renderStrategy.reset();
    }
