...
`;
    ...
    // UI documents are auto-generated by Design Job
    const uiSpecTemplate = `{
  "_comment": "ant:template - 디자인 잡 실행 시 자동 생성됩니다. Run 'ant design' to generate.",
  "placeholder": true
}
`;
    await fs.promises.writeFile(path.join(designOutputDir, 'ui-spec.json'), uiSpecTemplate, 'utf-8');

    const uiTokensTemplate = `{
  "_comment": "ant:template - 디자인 잡 실행 시 자동 생성됩니다. Run 'ant design' to generate.",
  "placeholder": true
}
`;
    await fs.promises.writeFile(path.join(designOutputDir, 'ui-tokens.json'), uiTokensTemplate, 'utf-8');

    const uiAssetsTemplate = `{
  "_comment": "ant:template - 디자인 잡 실행 시 자동 생성됩니다. Run 'ant design' to generate.",
  "placeholder": true
}
`;
    await fs.promises.writeFile(path.join(designOutputDir, 'ui-assets.json'), uiAssetsTemplate, 'utf-8');
