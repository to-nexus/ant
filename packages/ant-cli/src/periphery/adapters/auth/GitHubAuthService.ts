...
// Hard ceiling on every direct GitHub REST fetch — without this, a stalled
// TLS handshake or proxy idle-drop pins the caller (and the surrounding
// Redis lock) for the lock's full TTL. 30s is well above GitHub's p99
// response time so legitimate requests are unaffected.
...
export class GitHubAuthService {
  ...
  async createRepo(
    ...
    try {
      ...
      if (ownerResponse.ok) {
        const ownerInfo = await ownerResponse.json() as { type?: string };
        isOrg = ownerInfo.type === 'Organization';
      }
    } catch {
      // If we can't determine owner type, default to user account
      console.warn(`[GitHubAuthService] Could not determine owner type for ${owner}, defaulting to user account`);
    }

    // Use the appropriate endpoint: /orgs/{org}/repos for organizations, /user/repos for users
    const apiUrl = isOrg
