...
/**
 * The status row and the live-run chip row on step/gate cards are RESERVED
 * whether or not a run is at the step: a run appearing, moving or leaving must
 * not change the card height, or the structure key would flip and the canvas
 * would re-fit mid-run (doc 46 §7).
 */
export const STATUS_ROW_HEIGHT = 18;
...
export interface NodeMetricsInput {
  primary: string;
  caption?: string;
  /** Trigger cards carry no status/chip rows (they show a live count inline). Absent = step. */
  kind?: 'trigger' | 'step' | 'gate';
}

export function estimateNodeHeight({ primary, caption, kind }: NodeMetricsInput): number {
  const primaryLines = Math.max(1, Math.ceil(primary.length / PRIMARY_CHARS_PER_LINE));
  const runRows = kind === 'trigger' ? 0 : STATUS_ROW_HEIGHT + RUN_CHIP_ROW_HEIGHT;
  return CHROME + primaryLines * PRIMARY_LINE_HEIGHT + (caption ? CAPTION_HEIGHT : 0) + runRows;
}
