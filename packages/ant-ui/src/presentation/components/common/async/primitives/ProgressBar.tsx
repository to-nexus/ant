 ...
 *
 * The `ambient-progress` keyframe is defined in `src/styles/aurora-tokens.css`.
 * If the animation appears broken, verify that the keyframe entry still
 * exists there.
 */
export function ProgressBar({ active, className = '' }: ProgressBarProps) {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-y-0 left-0 w-1/3 bg-blue-500/70"
        style={{ animation: 'ambient-progress 1.2s ease-in-out infinite' }}
      />
    </div>
  );
}
