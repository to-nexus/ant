...
**Service Virtualization prompt SSOTs (MECE)** — three orthogonal partials cover the SV surface, do NOT merge them. Abstract concept = "Service Virtualization" (umbrella); leaf vocabulary = "mock" (env var / adapter / fake body):

| Partial | Scope | Activation |
|---------|-------|------------|
| [`service-virtualization-contract`](packages/ant-cli/src/core/prompt/templates/jobs/code/base/injections/service-virtualization-contract.md) | Port shape & toggle grammar (Service Virtualization core) | code job × `state.virtualizationSnapshot.hasBusinessConnection===true`, Handlebars-gated `{{> }}` in plan + execute rules.md |
| [`service-virtualization-data`](packages/ant-cli/src/core/prompt/templates/jobs/code/base/injections/service-virtualization-data.md) | Fake body realism (text/number/date/id/relation) | feature/ui/design-system task × hasBusinessConnection, gated |
| [`service-virtualization-imagery`](packages/ant-cli/src/core/prompt/templates/jobs/code/base/injections/service-virtualization-imagery.md) | Image subtype dispatch (3 pathways) | service ∧ hasFrontend ∧ taskType==='feature', gated |
| `ui-source-*` (`ui-assets.json`) | Design-system assets (logo, icon, decorative illustration) — orthogonal, NOT virtualization | per-source via `ui-source-dispatch` (ui/design-system tasks) |

Gate predicates live under [`core/prompt/builder/serviceVirtualization/`](packages/ant-cli/src/core/prompt/builder/serviceVirtualization) — three pure functions plus the `buildVirtualizationSnapshot` resolve-time helper that fills `state.virtualizationSnapshot.hasBusinessConnection`. Regression guards: [tests/prompt/service-virtualization.test.ts](packages/ant-cli/tests/prompt/service-virtualization.test.ts) locks body discipline, wire sites, gate truth tables, and Handlebars render gates; [tests/policy/service-virtualization-vocabulary.test.ts](packages/ant-cli/tests/policy/service-virtualization-vocabulary.test.ts) locks the SSOT-table title across `.cursorrules`, `CLAUDE.md`, and the rubric.
...
- `ANT_VECTOR_DB_ENABLED`: Vector DB capability toggle (default: `false`). When unset/false, [`AdapterFactory.createMemoryAdapter`](packages/ant-cli/src/infrastructure/adapters/AdapterFactory.ts) returns a [`NoopMemoryAdapter`](packages/ant-cli/src/periphery/adapters/memory/NoopMemoryAdapter.ts), the architect/learn job + `ant index` CLI + git auto-index short-circuit, and `/agents` drops the `learn` entry. RAG degrades gracefully — `vector → git-changes → keyword` chain in [`semanticSearch.ts`](packages/ant-cli/src/agents/architect/graph/code/nodes/plan/semanticSearch.ts) yields the same total quota using git + grep alone. SSOT: [`vectorDbCapability.ts`](packages/ant-cli/src/core/config/vectorDbCapability.ts) — never read `process.env.ANT_VECTOR_DB_ENABLED` elsewhere. To enable, set `ANT_VECTOR_DB_ENABLED=true` and run `pnpm dev:infra:vector`.

  **Invariant (Strict)**: every vector-DB entry point gates on `isVectorDbEnabled()` at the function's first line. The composition root ([`orchestrator.ts`](packages/ant-cli/src/composition/orchestrator.ts)) wires `memory = isVectorDbEnabled() ? AdapterFactory.createMemoryAdapter() : undefined` so consumers receive a real `MemoryPort` or `undefined` — **never a NoOp imposter inside the normal flow**. NoOp fallbacks (`?? AdapterFactory.createMemoryAdapter()`) inside graph nodes are forbidden — past the gate, `state.deps.memory` is guaranteed real; missing deps must throw a DI-bug error. Active gates live in: [`orchestrator.ts` learn branch](packages/ant-cli/src/composition/orchestrator.ts), [`memory/index.ts:retrieve`](packages/ant-cli/src/agents/architect/memory/index.ts), [`learn/nodes/resolve.ts:executeIndexing`](packages/ant-cli/src/agents/architect/graph/learn/nodes/resolve.ts), [`learn/nodes/store.ts`](packages/ant-cli/src/agents/architect/graph/learn/nodes/store.ts), [`cli/index-command.ts`](packages/ant-cli/src/cli/index-command.ts), [`GitService/indexing/index.ts:autoIndexCodebase`](packages/ant-cli/src/periphery/adapters/http/services/GitService/indexing/index.ts), [`BaseGitSetupOperation.ts:triggerIndexing`](packages/ant-cli/src/periphery/adapters/http/services/GitService/remote/operations/BaseGitSetupOperation.ts), [`health.routes.ts`](packages/ant-cli/src/periphery/adapters/http/routes/health.routes.ts). New entry points must add the same gate before any `MemoryPort` / `ChunkPort.process` / `Indexer.index` call. The `NoopMemoryAdapter` exists only as a last-resort safety net for capability-unaware helpers (e.g. `triggerIndexing` 's clear), not as a normal-flow substitute.
...
위 명령으로 수동 sweep 한다. 디스크 잔존 옛 트리는 더 이상 자동 마이그레이션되지 않으므로, 발견 시 수동으로 정리한다.
...
## Three-Axis Task Modeling SSOT — type / band / priority

**Task 분류는 세 직교 axis 로 분리된다. 각 axis 의 관찰자가 다르므로 한 field 에 두 책임을 묶지 않는다.**

| Axis | 관찰자 | 결정하는 것 | 단위 |
|---|---|---|---|
| `task.type` | **LLM** | "이 task 에서 무엇을 하라" — 행동 모드 | `feature` / `error` / `verification` / `ui` / `design-system` / `test-code` / `doc` / `setup` / `explain` |
| `task.band` | **Orchestrator** | feature type 안에서의 scheduling 위치 (type-bound to FeatureTask) | `'foundation'` / `'integration'` / `undefined` (= 일반 feature) |
| `task.priority` | **TaskQueue** | 정렬상 몇 번째인가 | 정수 (의미적 사용 금지) |

### 분리 기준 — type 으로 vs band 로

```
이 분류 차이가 prompt 본질에 영향을 주는가?
├─ YES → 새 type 으로 분리
│   예: design-system (token 인프라) 은 feature (코드 작성) 와 prompt 본질 다름.
│
└─ NO  → 같은 type, band 로 분리
    예: foundation feature 와 integration feature 는 둘 다 "feature 코드 작성"
        이고 prompt 본질 동일. 큐 위치만 다름 → band 로 분리.
        Orchestrator 만 변별하는 axis.
```

### priority 의 의미적 사용 금지

`task.priority` 는 **`TaskQueue.push()` 의 정렬 비교** 가 *유일한 합법 사용처*. 모든 다른 곳 (phase node / router / parallel / classify / barrier / type 결정) 에서 priority 의 비교는 금지된다.

**유일한 priority → 의미 변환 site** 는 `decompose/responseParser.ts::deriveBandFromPriority` — LLM 이 emit 한 priority 를 보고 feature task 에 `band` 를 붙인다. 그 이후 모든 site 는 `task.band` (feature) 또는 `task.type` (다른 변종) 만 본다.

bundle 코드 (`tasks/{type}/hooks/scheduling.ts`) 는 phase 코드 가 아니라 SSOT 자체이므로 priority 를 읽어도 된다 — `doc` bundle 의 design-job tokens/foundation 분기가 그 예. 단, 그 bundle 의 priority 해석 결정이 phase 로 누수되지 않는다 (classify 출력 flag 만이 phase 와 인터페이스).

### Discriminated Union 강제

`@ant/shared/task.ts` 의 `BaseTask` 는 `type` 으로 discriminated union — `band` 는 `FeatureTask` 한 변종에만 존재한다. 따라서:

- `if (task.type === 'verification') { task.band }` — **컴파일 에러** (narrow 후 VerificationTask 에 band 없음)
- `{ type: 'verification', band: 'foundation', ... }` — **컴파일 에러**
- `{ type: 'feature', band: 'foundation', ... }` — OK
- `{ type: 'feature' }` (band 생략) — OK, "일반 feature"

### batchSplit Path 별 priority 의미

`tasks/_shared/batchSplit/process.ts` 의 sub-task 는 parent 의 path 별로 priority 가 다르다:

| parent path | sub priority | 이유 |
|---|---|---|
| `requeue-parent` (verification → error) | `parent − 1` | parent 가 큐에 남아있어야 sub 가 먼저 dequeue. priority 차이로 ordering 보장. |
| `drop-and-replace` (error / test-code / feature / ui) | `parent` | parent 가 큐에서 제거됨. priority 보존이 band classification 안정성에 필수 (foundation parent@200 → sub@200 stays foundation). |

**`band` 는 양 path 모두에서 parent verbatim carry-over** — feature parent 의 `band` 가 그대로 sub 에게 상속된다. 이게 `log-satin-feeling-orbit` deadlock 의 근본 fix: 사전엔 sub priority decrement 가 foundation window (200-299) 를 벗어나면서 classify (priority-based) 가 ordinary feature 로 잘못 분류 → orchestrator foundation gate 가 reject → 모든 worker 종료 + 큐 정체.

### ❌ ABSOLUTELY FORBIDDEN

- 새 phase 코드에 `task.priority === N` / `task.priority < N` / `task.priority >= TASK_PRIORITIES.X` 같은 의미적 priority 비교 추가. 정렬 (queue push 의 sort callback) 만 합법.
- `band` 를 non-feature task 에 박는 시도 (discriminated union 이 차단).
- decompose 외 다른 phase site 에서 priority → band 매핑 helper 신설.
- batchSplit Path B 가 sub priority 를 `parent − 1` 로 떨어뜨리는 옛 동작 부활. 또는 Path A 가 sub priority 를 `parent` 로 같이 놓는 변형 (queue ordering 깨진다).

### ✅ CORRECT

- 모든 phase 분류 dispatch 는 `hooksForTaskType(t.type)?.scheduling?.classify?.(t)` (반환 flag 로 분기). bundle 가 SSOT.
- 새 task 종류 추가는 `TaskType` union + 새 bundle (`tasks/{type}/`) + registry entry 한 줄 — phase 코드 변경 0.
- feature 의 새 scheduling 위치 추가 (예: `'staging'`) 는 `TaskBand` union 한 줄 + decompose mapping + feature classify 분기. type 신설 불필요.
- decompose 의 `deriveBandFromPriority` 가 priority 표 변경 시 갱신되는 단일 site.

### Why This Matters

`log-satin-feeling-orbit` deadlock (May 4 2026) 의 RCA: foundation parent (priority 200) 가 batch-split Path B 로 priority 199 sub-task 들을 큐 앞에 넣음 → classify (당시 priority window 기반) 가 199 를 ordinary feature 로 분류 → orchestrator 의 `hasPreFeatureWork && !isFoundation && !isTokens` gate 가 sub-task 거절 → 모든 worker 종료 + spawn 트리거 부재 → 60s 체크포인트만 영원히 같은 큐 사이즈 토해냄.

핵심 통찰: **classify 의 input 을 priority 에서 explicit `band` field 로 옮기면 priority 변환이 분류를 흔들 수 없다.** Three-Axis 가 진정한 D31 ("phase 가 task.type / priority 직접 비교 안 함") 의 실현이다.

### Enforcement

```bash
# 1. 산발 priority 비교가 phase 레이어에 부활하지 않았는지.
#    `decompose/responseParser.ts` 와 `tasks/_shared/batchSplit/process.ts`
#    (Path A `Math.max(1, parent - 1)` 클램프 — 정렬 산술) 가 유일한
#    합법 site 이므로 검증 시 두 파일은 단독으로 보고 의미적 비교 0 인지 확인.
rg -n "\.priority\s*[<>!=]=?\s*(\d+|TASK_PRIORITIES\.)" \
  ...
  packages/ant-cli/src/agents/architect/graph/ask/nodes \
  packages/ant-cli/src/agents/architect/graph/learn/nodes \
  packages/ant-cli/src/agents/common/graph \
  --glob '!**/decompose/responseParser.ts'
# Expected: 0 matches.

# 2. priority → band 매핑이 단일 site (decompose) 에 있는지
rg -n "deriveBandFromPriority" packages/ant-cli/src
# Expected: definition + import in responseParser only.

# 3. 회귀 가드 — 새 deadlock 시그니처가 fail 시그널을 내는지
pnpm vitest run tests/parallel/orchestrator-deadlock-regression.test.ts \
  tests/verification/unit/batchSplitBandCarryover.test.ts \
  tests/policy/priority-isolation.test.ts \
  --reporter=dot
# Expected: all pass.
```

회귀 테스트: [`packages/ant-cli/tests/parallel/orchestrator-deadlock-regression.test.ts`](packages/ant-cli/tests/parallel/orchestrator-deadlock-regression.test.ts), [`packages/ant-cli/tests/verification/unit/batchSplitBandCarryover.test.ts`](packages/ant-cli/tests/verification/unit/batchSplitBandCarryover.test.ts), [`packages/ant-cli/tests/policy/priority-isolation.test.ts`](packages/ant-cli/tests/policy/priority-isolation.test.ts).
...
- 도메인 전환 시 자산은 사용자 책임으로 이동 — 시스템은 조용히 한 쪽만 활성화한다 (자동 재배치 없음)
...
## Project / Feature Lifecycle SSOT — repoType / cleanup cascade / cross-process pubsub

**프로젝트·피처 라이프사이클은 세 정책의 합집합이다. 셋 모두 위반하면 "Project already exists" 회귀 / git worktree path-collision / preview 누수가 재발한다.**

### ❌ ABSOLUTELY FORBIDDEN

- `userContext` (`local:local` vs cloud) 로부터 `repoType` 자동 매핑 부활. `repoType: 'local'` 진입은 사용자가 wizard advanced 옵션에서 명시 입력했을 때만 합법. `isCloudMode` / `mode === 'local' ? 'local' : 'cloud'` 등 자동 분류 패턴은 BE / FE 어디에도 두지 않는다 (path-collision 의 근본 원인).
- `ProjectService.deleteProject` 의 cascade 단축: K8s pod 종료 대기 (`waitForPodDeletion`) → preview pub/sub ack → child exit 대기 → Redis state cleanup → fs.rm verification 5 단계 중 하나라도 건너뛰면 EFS open file handle 이 NFS silly-rename `.nfsXXXX` orphan 을 남기고, 다음 createProject 가 stale projectPath 를 보고 409 를 던진다.
- Cross-process cleanup 을 in-process 호출로 처리. `PreviewService` 는 `ant-preview` 별도 프로세스에 살아있으므로 `ProjectService` 가 직접 메소드 호출하면 빈 Map 만 만진다. **반드시** Redis pub/sub 채널 `ant:lifecycle:cleanup:request` / `ant:lifecycle:cleanup:ack` (`REDIS_KEYS.LIFECYCLE.CLEANUP_*`) 으로 흘리고, publisher 는 ack 매칭 전까지 await.
- `WorktreeService.createWorktree` / `removeWorktree` 가 `mainCodebasePath === worktreePath` 가드 없이 실행. `repoType:'local'` 모드에서는 base 와 모든 feature 가 같은 codebase 를 공유하므로 가드 없이 `git worktree add` 를 호출하면 메인 repo 자체에 worktree 를 만들어 .git 디렉토리를 손상시킨다.
- `ideOrchestrator.stop().catch(() => undefined)` / `c.remove(...).catch(() => undefined)` 같은 silent partial-delete 패턴. cleanup 경로 (`IDEService.cleanupProject`, `ProjectService.deleteFeature`) 안에서는 실패가 surface 되어야 한다 (regression guard: `tests/cleanup/silent-catch-removed.test.ts`).
- K8s pod DELETE 시 grace period 명시 누락 + `waitForPodDeletion` timeout == grace period 인 경계 충돌. `gracePeriodSeconds: 5` (DELETE body) + `TIMEOUTS.POD_DELETION = 60_000` 이 SSOT.

### ✅ CORRECT

- `repoType` 기본값은 항상 `'cloud'` (BE `ProjectCrudService.createProject` / `getProjectConfig` fallback, FE `createProjectConfig`). 사용자 명시 입력 채널만 `'local'` 진입.
- `K8s IDE pod` 의 mount topology 는 alias 모델 — workspace mount 는 `mountPath: '/workspace'` 유지, 헬퍼 (`resolveK8sWorktreeMounts`) 가 추가 mount (mainGitDir + worktreePath) 를 절대경로로만 spread, `container.workingDir = '/workspace'` 가 기본 폴더 (Docker `WorkingDir` 와 1:1 동형).
- `ANT_WORKSPACE_BASE_PATH` 불일치 시 `createPodSpec` 진입에서 fail-fast throw — silent broken pod 차단.
...
```bash
# 1. Auto-local 분류 (BE) / worktree 가드 잔존 검증
pnpm vitest run tests/policy/git-bootstrap-ssot-regression.test.ts --reporter=dot
# Expected: 6 passed (legacy 4 + Phase 3.4 BE-side 2)

# 1b. FE 자동 매핑 가드 (ant-ui) — BE 가 FE 파일을 가로질러 읽으면 Docker 빌드가
# 깨지므로 FE 정규성 검사는 ant-ui 슈트가 소유한다.
cd packages/ant-ui && pnpm vitest run tests/git-world/repotype-default.test.ts --reporter=dot
# Expected: 2 passed

# 2. K8s mount 회귀 가드
pnpm vitest run tests/ide/ --reporter=dot
# Expected: 9 passed (k8sWorktreeMounts.test.ts + k8sPodSpec.test.ts)
