...
type DirectiveType = typeof DIRECTIVE_TYPES[keyof typeof DIRECTIVE_TYPES];
type AgentMode = 'design' | 'code' | 'learn';

// 디렉티브 읽기 함수
function readDirective(directivesPath: string, type: DirectiveType): string | null {
  const typePath = path.join(directivesPath, type);
  if (!fs.existsSync(typePath)) return null;

  // 일반 디렉티브 파일 찾기 (directive-N.md)
  ...
  if (files.length > 0) {
    return fs.readFileSync(path.join(typePath, files[0]!.name), "utf8");
  }
...
export async function architectAgent(
  ...
  if (mode === 'learn') {
    // === Stage 0: Learn based on directive ===
    let featureFolder = "";
    if (inputFile) {
    ...
    const learnDirective = readDirective(path.join(process.cwd(), "projects", project, featureFolder, "directives"), DIRECTIVE_TYPES.LEARN);
    if (!learnDirective) {
      throw new Error("No learning directive found. Create a directive in directives/learn/ first.");
    }
...
`;
    ...
    const reportFile = path.join(reportDir, `learning-report-${Date.now()}.md`);
    fs.writeFileSync(reportFile, reportContent, "utf8");
    
    console.log(`📊 Learning report saved: ${reportFile}\n`);
...
`;
    ...
    if (featureFolder) {
      ...
      // Try to read latest directive based on mode
      const directivesPath = path.join(process.cwd(), "projects", project, featureFolder, "directives");
      if (fs.existsSync(directivesPath)) {
        if (mode === 'code') {
          codeDirectiveContent = readDirective(directivesPath, DIRECTIVE_TYPES.CODE) || "";
        } else if (mode === 'learn') {
          codeDirectiveContent = readDirective(directivesPath, DIRECTIVE_TYPES.LEARN) || "";
        }
      }
    }
