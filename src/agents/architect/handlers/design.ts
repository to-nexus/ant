...
`;
  ...
  // 3. 리포트 생성 (디자인 프로세스에 대한 메타 정보)
  const reportContent = `# Design Process Report
**Project:** ${context.project}
...
- Previous Design: ${previousDesign ? 'Yes' : 'No'}
- Design Directive: ${directive ? 'Yes' : 'No'}
...
`;
  ...
  // 리포트에 학습 내용 추가
  const finalReportContent = reportContent.replace(
    '---\n*This report',
    `\n## Extracted Knowledge\n${learnings}\n\n---\n*This report`
  );

  const reportFile = generateReport("design", context, finalReportContent);
  ...
  console.log(`\n✅ Design document saved: ${reportFile}`);
