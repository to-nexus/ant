...
| `universal`   | universal | Chat + files under `universal/artifacts/` (custom agent/job runtime) | Custom job run in a universal-type project |
...
projects, skips triage, and rides the workspace's single chat container
(`{project}/universal`) instead of features — sessions are kept per
(agent, job) pair.
