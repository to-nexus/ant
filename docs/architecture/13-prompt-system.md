...
예: `gen-code-sys` intent + `visual/ui/ant` 경로에 UI 설계 문서가 있으면 → `ui-design-policy` 주입. 세 UiSource 별 해석 규약은 `ui-source-dispatch` 를 통해 `ui-source-{ant,figma,handoff}.md` 중 하나로 라우팅된다.
