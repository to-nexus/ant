...
| `ANT_CODESPACE_ENABLED` | `true` | The ONE switch for the code-generation surface. `false` (also `0`/`no`/`off`) applies two consequences together: the Cloud IDE is not mounted (no `/ide` proxy, no `/api/cloud-ide/*`, no orchestrator, no Docker-socket / K8s-client contact at boot) and only `universal` (custom-agent) projects can be created, re-typed, or run — a canonical create/PUT/job answers 400 `project-kind-disabled`. Canonical projects created before the switch are hidden from the project list and the reference catalog (the directory is kept, not deleted; flip the switch back to see them). Reported as `capabilities.codespace`. There is deliberately no per-surface sibling (no IDE-only flag, no project-kind list). SSOT: `core/config/codespaceCapability.ts`. |
