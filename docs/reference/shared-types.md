...
| Type / function | Source | Purpose |
|-----------------|--------|---------|
| `PipelineDef` / `PIPELINE_DEF_VERSION` / `resolveRunConcurrency` | `pipeline.ts` | One definition: `version`, `name`, `on` (`schedule` and/or `upstream`, or `fetch` alone, or none = manual), `concurrency?` (live runs per activation, `1..PipelineCaps.maxLiveRunsPerActivation`; `resolveRunConcurrency` is the one reader, every trigger shares the cap), `defaults?`, `steps[]`, `acknowledged?`. Project-free by construction — `projectId` was a v1 field and is now rejected. |
...
| `PipelineUpstreamTrigger` / `PipelineRunUpstream` / `upstreamTemplateVars` | `pipeline.ts` | The pipeline→pipeline edge: `pipelineId`, `step?` (absent = the run seal as a node), `when?: StepEdgeCondition` (default `success`; `verdict:*` needs `step`), `overlap?`. `PipelineRunUpstream` is the sealed node frozen onto an event-fired `RunRecord.upstream` (`{{trigger.upstream.*}}`, directive-only); `upstreamTemplateVars` is the one derivation of that vocabulary. |
| `PipelineFetchTrigger` (`PipelineFetchBoundTrigger` \| `PipelineFetchInlineTrigger`) / `PipelineFetchConnection` / `fetchConnectionSource` / `PipelineRunItem` / `PipelineFetchStatus` / `parseItemPath` / `fetchItemTemplateVars` | `pipeline.ts` | The pull trigger: a REST connection in exactly one of two forms — BOUND (`customJobRef` + `api`, a job's declared `apis` entry resolved in the activator's scope) or INLINE (`connection: { baseUrl, headers? }`, the external `apis` shape minus `allow`/`self`; `fetchConnectionSource` is the one discriminator) — a read-only `request` (trigger configuration — never rendered to a model, never a tool), item-paths `items` / `key` / `fields?` (`$`, `.name`, `['name']`, `[n]`; no wildcards), `every`. `PipelineRunItem` is the claimed case frozen on the run; `PipelineFetchStatus` is last-poll telemetry, never a judgment input. `fetchItemTemplateVars` derives the second template grammar (`trigger.item.key` + declared fields) from one definition. `formatItemPath` is `parseItemPath`'s inverse (the editor writes picked paths with it). |
| `PipelineFetchPreview` / `PipelineFetchSampleNode` / `validatePipelineFetchProbe` / `validatePipelineFetchSelection` | `pipeline.ts` | The `preview-fetch` reply: `ok` + `sample` say the REQUEST reached a JSON body (a bounded tree — arrays keep `total`, objects `more`, strings `cut`), `mapping` says whether the SELECTION picked items; the two validators are the two halves of the fetch rules the route judges separately. |
...
| `RunRecord` / `StepRecord` / `GateRecord` / `ClarifyRecord` / `PipelinePendingApproval` | `pipeline.ts` | Run history shapes, including the `awaiting_clarify` step state, the case `item?` (a fetch item or a discovered case — the same channel), a case run's `discoveryRunId?` / `discoveryStepId?`, and a discovering step's sealed `StepRecord.cases?`. Gate routing rides `GateRecord.assignees?` (⊆ candidates; `assigneeSource` / `assignedBy`), sourced from a human reassign or the upstream `StepRecord.assignee?` (the sealed `<assignee>` nomination, kept only when it names a candidate). Inbox rows carry `assignees?` + `candidates?` for sorting and the reassign select — a routing hint, never a permission: any candidate still decides. |
| `PipelineRunEvent` / `PipelineFiredBy` | `pipeline.ts` | The run-log line and SSE payload the Pipelines tab renders live (`fired`, `item_claimed`, `awaiting_human`, `gate_reassigned`, …). `PipelineFiredBy` = `cron` \| `manual` \| `event` \| `fetch` \| `discovery`; the FE keys its icon/label tables as `Record` over it (`Pipelines/runIdentity.ts`). |
| `StepDiscoveryDef` / `StepDiscoveryMissingPolicy` / `discoveryItemTemplateVars` / `discoveryStepIndex` / `perCaseStepIds` | `pipeline.ts` | Step discovery — the delegated executor beside `on.fetch`: `JobStepDef.discovers` declares the case FIELD vocabulary (`fields?`, the `outcomes` precedent — never the mechanism) and `onMissing?` (`fail` default \| `complete`); `discoveryStepIndex` names the one discovering step, `perCaseStepIds` its needs closure (the steps a case run executes; a discovery run pre-skips them), `discoveryItemTemplateVars` the `{{trigger.item.*}}` vocabulary those steps may reference — the ONE derivation the validator, the prompt band and the FE token picker share. `UniversalTurnMeta.caseFields` (`custom-agents.ts`) carries the vocabulary to the turn; coordinator-only. |
