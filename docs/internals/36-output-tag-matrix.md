...
| `<cases>` | metadata | consumed-formatted + post-stream | chat-line + sealed-state | non-blocking | universal agent on a pipeline step declaring `discovers` (the Case Discovery band) — the discovered cases, a JSON array of `{key, fields?}`; parse SSOT `core/pipelines/cases.ts::parseCaseNominations` (LAST tag, fetch-item bounds), sealed as `cases` (or `casesError` for an unreadable body); the coordinator filters to the declared fields, queues every case into the claim ledger and fans the per-case steps out one run per case (doc 46 §2 Discovery). Chat shows the count only |
