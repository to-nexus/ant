...
|---|---|---|
| design plan | `<plan>` `<thinking>` | "`<plan>` 은 봉인 JSON. plan 노드는 `</plan>` 직후 종료 — 후행 narrative 없음. 접근 전략은 후속 execute 의 `<reply>` 에서 표명." |
| design execute (spec) | `<file>` `<append>` `<reply>` `<done>` `<clarify>` `<thinking>` | "spec 본문은 반드시 `<file>` 안. 결정 요약은 `<reply>` 한 번." |
| design execute (system / ui-design / game-art-design) | 위와 동일 | variant 별 본문 포맷만 보강 |
