...
- **전 Phase 완료** — P0·P1·P2·P3·P4 (ant) + P5·P6·P7 (ant-cloud) + H (DevOps 핸드오프 enumerate). 다음 작업 없음 — 남은 것은 (a) 사용자 확인 후 push/배포, (b) C8 ant 커밋 push 후 ant-cloud submodule SHA bump, (c) 아래 caveat 의 런타임 실증.
- **ant-cloud 커밋 상태** — `ant-cloud` `main` 직접, **미푸시**(push 는 사용자 확인 후):
  | 커밋 | 내용 |
  |---|---|
  | `b298795` | P5 (workflows SHA 핀 + permissions + SECURITY.md) |
  | `216c133` | P6 (deploy.yml prod ticket 가드 + auth.routes.ts 디버그 게이트 + 테스트 6) |
- **커밋 상태** — `ant` `main` 직접, **전부 미푸시**(push 는 사용자 확인 후 — main push = dev 자동배포 트리거):
  ...
  | `b5e6b243`/`4050c956` | P4/P5 핸드오프 기록 |
  | `7437e683` | P7 C8 tenant-path-isolation 테스트 (5) |
  | (this) | P6/P7/H 핸드오프 갱신 + boot-smoke CI 키 런타임 생성 수정 (ci.yml) |
- **C8 submodule 반영**: C8 코드/테스트의 home repo 는 **ant**(`WorkspacePathResolver.ts` = OSS 공유코드). ant-cloud 는 ant main push 후 `ant/` submodule SHA bump 로만 반영(별도 코드변경 없음) — push 시점에 함께.
...
- **이월/별도 레포 TODO**: ① ~~`ant-cloud/SECURITY.md`~~ **완료**(P5 `b298795`). ② docker build 실증(P2, 로컬 데몬 down 으로 정적검증만) — 데몬 가용 시 cli/ui/ide 빌드 green 재확인 권장. ③ **P4 O8 securityContext 런타임 실증**: openvscode-server pod 가 uid1000 로 EFS subPath 쓰기 가능한지 — 정적 필드만 검증, 클러스터 미실증. ④ **C8 submodule SHA bump**(위 참조, push 시점). ⑤ **H DevOps 항목**(§H) 실제 인프라 반영은 DevOps 책임 — 본 작업은 enumerate/전달까지.
- **검증 명령 재현**: ant 루트 `env -u GITHUB_TOKEN -u GH_TOKEN pnpm test:cli`(P4 시점 **4919 passed**, +C8 5 = **4924**), `pnpm build:cli`·`pnpm build:ui`; ant-cloud `packages/cloud` `pnpm typecheck` + `pnpm test`(**94 passed**, +C4 6); ant-cloud 루트 `gitleaks detect --no-banner`(**no leaks**, 58 commits).
...
- 2026-06-30 **P4 완료(미커밋)** — 앱-레이어 4픽스 + 테스트 14. **O6** [CredentialsStore.ts:209-226](packages/ant-cli/src/utils/userConfig/CredentialsStore.ts): `ANT_ENCRYPTION_KEY` 가 set 인데 length≠64 또는 non-hex 면 silent fallback(파일/랜덤키) 대신 **throw** — 잘못된 키로 자격증명을 암호화해 이후 올바른-키 부팅이 복호 못 하는 footgun 차단. env 미지정 시의 파일/생성 fallback 은 보존. **O7** [httpProbe.ts:24-46,56-86](packages/ant-cli/src/agents/common/tool/handlers/httpProbe.ts): 절대 URL SSRF 게이트 — `isLoopbackHost`(localhost/127.0.0.0/8/::1/0.0.0.0)만 허용, 메타데이터 IP(169.254.169.254)·RFC-1918·공개호스트·malformed reject. `runningServers` 가 port 만 보유(전부 loopback bind)하므로 정당 타깃은 loopback 뿐 — handoff 의 "등록 프리뷰 호스트" 는 추적 채널이 없어 loopback-only 로 정착. 상대 URL 경로 불변. **O8+O12** [KubernetesIDEOrchestrator.ts:391-417](packages/ant-cli/src/infrastructure/ide/KubernetesIDEOrchestrator.ts) `createPodSpec`: pod-level `automountServiceAccountToken:false`(O12) + pod/container `securityContext`(`runAsNonRoot:true`/`runAsUser:1000`/`runAsGroup:1000`; container 추가 `allowPrivilegeEscalation:false`/`capabilities.drop:['ALL']`). `K8sPod` 타입에 `K8sSecurityContext` 인터페이스 + pod-spec/container 필드 추가. **정적 필드만 검증** — openvscode-server 이미지 기본 user 가 uid1000 이라 무변경 기대이나 EFS subPath 쓰기 런타임 실증은 클러스터 가용 시(이월 TODO ③). **O9** [userContext.ts:8-22,52-67,118-120](packages/ant-cli/src/periphery/adapters/http/routes/helpers/userContext.ts): 로컬모드 tenant 추론이 multi-org/user 로 ambiguous 면 1회 `logger.warn`(동작 불변 — 여전히 null→`local` fallback). `__resetInferredLocalDefaultForTests` 가 warn flag 도 리셋. **테스트**: `tests/http/http-request-handler.test.ts`(SSRF 6 추가)·`tests/utils/credentials-store-key.test.ts`(4 신규)·`tests/ide/k8sPodSpec.test.ts`(securityContext 1 추가)·`tests/http/local-tenant-ambiguity-warn.test.ts`(3 신규). **검증**: `tsc --noEmit` clean, `pnpm test:cli` **4919 passed**(8 skip, 직전 4905 +14). 다음=P5(ant-cloud).
- 2026-06-30 **P5 완료(ant-cloud `main` 직접, 미푸시)** — 커밋 `b298795`. **C1 SHA 핀**: `to-nexus/cross-hub-workflows@master` 9곳(deploy.yml ×7 = security-scan-v2 ×1 + ant-cloud.yml ×6, build-ide-image.yml:29, pr-security-check.yml:13) → `@1bd595e5204143a8bee27bcf74ca7e9763ebdaf0`(master HEAD 2026-06-30), `peter-evans/create-pull-request@v6` → `@c5a7806660adbe173f04e3e038b0ccdcd758773c`. 1st-party(`actions/*`·`pnpm/*`·`aws-actions/*`)는 `@vN` 유지(결정 4). 각 핀에 re-pin gh 명령 인라인 주석. **C5 permissions(reusable 요구사항 실측 기반)**: 재사용 워크플로 `security-scan-v2.yml` 이 자체적으로 `permissions: contents:read + actions:write` 선언 → 이를 호출하는 job 은 `actions:write` 필수(제거 불가). `ant-cloud.yml`(AWS 배포)은 `id-token:write` 필요. 따라서 top-level 을 `contents:read` 로 최소화하고 job-level 로 승격: security-scan job=`actions:write`, AWS/reusable 배포 job 10개=`id-token:write`, detect-changes/test-ant-cli/secret-scan=read-only 상속. update-submodule(단일 job)=`contents:write`+`pull-requests:write` 유지(둘 다 필요). 검증: js-yaml 파싱 4/4 OK, **per-job 권한이 uses/steps 실측 needs 와 정확히 일치**(over-grant 0, missing 0). **C6**: `SECURITY.md`(P0 생성분, 멀티테넌트 위협모델/리포팅/supply-chain 표준/SLA) 커밋. ⚠️ **cross-hub-workflows 는 to-nexus 내부 repo 지만 `@master`(mutable ref)라 supply-chain 결정상 핀 대상** — 내부 워크플로 업데이트는 re-pin 필요(주석의 gh 명령). dev 배포(auto-dev-*)가 prod(manual-*)보다 먼저 fire 하므로 권한 스코핑 회귀의 canary 역할. 다음=P6(deploy ticket 가드 + auth 디버그 로깅).
- 2026-06-30 **P6 완료(ant-cloud `main` 직접, 미푸시 — 커밋 대기)** — deploy 가드 + auth 디버그 로깅. **C3** [deploy.yml](../../../ant-cloud/.github/workflows/deploy.yml): 신규 `validate-prod-deploy` job(`if: workflow_dispatch`) — `environment==prod && ticket_id==''`이면 `::error::` + `exit 1`. **5개 manual-* job 전부** `needs:` 에 `validate-prod-deploy` 추가(가드 실패 시 dependent skip = prod 배포 차단). 단일 가드 job(SSOT) — 각 job 인라인 step 복제 회피(`uses:` reusable job 은 step 추가 불가하므로 needs-gate 가 유일 수단이기도 함). required-reviewers 는 코드 외 → H. **C4** [auth.routes.ts](../../../ant-cloud/packages/cloud/src/routes/auth.routes.ts): 신규 SSOT `isAuthDebugLoggingEnabled()`(export) — `ANT_AUTH_DEBUG==='1'` AND **not prod-like**(`NODE_ENV==production` ∨ `ANT_SERVER_MODE==cloud`)일 때만 true. 두 디버그 로그 사이트(440·737, `/auth/me` cookiePresent/origin/host/xfp/xfh + `/auth/signout` hostname/rawCookie/setCookie)가 동일 게이트 사용 → prod 에서 raw 헤더 로깅 구조적 차단(env 오설정으로도 누출 불가). redact 대신 force-disable 선택(전부 제거가 더 안전). **검증**: deploy.yml js-yaml 파싱 OK(14 job, 5 manual needs 정합), 가드 셸로직 4-조합 테스트(prod+무티켓 FAIL / 나머지 PASS), `pnpm typecheck` clean, 신규 `tests/http/auth-debug-gate.test.ts`(6) green, ant-cloud 전체 **94 passed**(직전 88 +6, auth-me 회귀 무손상). 다음=P7.
- 2026-06-30 **P7 완료** — 히스토리 scrub 검증 + tenant isolation. **C2(clean)**: `gitleaks 8.30.1` `gitleaks detect --no-banner --redact`(전 히스토리) — ant-cloud 루트에서 **58 commits scanned, no leaks found**. `ant/` 서브모듈은 gitlink(포인터)라 gitleaks 가 descend 안 함 → ant-cloud 고유 커밋만 스캔(요구사항 "submodule 제외" 충족). purge/rotation 불요 — **clean 기록**. **C8(정상 — 테스트만 추가, home repo=ant)**: [WorkspacePathResolver.ts](packages/ant-cli/src/core/config/WorkspacePathResolver.ts) `UnifiedWorkspaceResolver` 4 메서드(workspace/project/feature/codebase) 전부 `path.join(base, organizationId, userId, ...)` — **org→user 순서로 둘 다 구조적 스코프**. org/user 는 서명검증된 JWT 에서 파생(`extractUserContext` → `req.organization.id`/`req.user.id`, cloud 모드 JWT 미들웨어). org 위조는 서명비밀 없이는 불가 + org 가 항상 첫 세그먼트라 타 org 서브트리 도달 불가 = 구조적 격리. **결함 없음 → 코드 수정 없이 회귀테스트만 추가**: 신규 [tests/workspace/tenant-path-isolation.test.ts](packages/ant-cli/tests/workspace/tenant-path-isolation.test.ts)(5) — org+user 순서 세그먼트, JWT org-forgery 격리(같은 user/proj/feature 라도 org 만 다르면 disjoint 서브트리, org-b 컨텍스트가 org-a 서브트리에 절대 미진입), org 내 user 격리. green. ⚠️ 수정 위치=ant(공유코드), ant-cloud 는 push 시점 submodule SHA bump 로만 반영. 다음=H.
- 2026-06-30 **P4 후속 — boot-smoke CI 키 수정** — GitHub Actions `boot-smoke` ([.github/workflows/ci.yml](.github/workflows/ci.yml))가 `ANT_ENCRYPTION_KEY: ci-boot-smoke-encryption-key-0001`(33자, non-hex)로 부팅 → **P4 O6 fail-fast 가 정확히 작동**해 `invalid length: 33 (expected 64)` throw, exit 7. (P4 검증이 로컬 `pnpm test:cli` 였고 boot-smoke 는 CI 전용이라 누락된 fallout — 결함 아닌 의도된 fail-fast 의 정당한 트립.) **수정**: 하드코딩 64-hex 리터럴 대신 **런타임 생성** — 새 step `echo "ANT_ENCRYPTION_KEY=$(openssl rand -hex 32)" >> "$GITHUB_ENV"`(boot step 직전). 이유: 커밋된 64-hex 리터럴은 **gitleaks 가 generic-api-key 로 flag**(실측: 하드코딩=leaks found 1, 런타임생성=no leaks). ephemeral local-mode boot 이라 per-run 랜덤키 무해(자격증명 미영속). 검증: `gitleaks dir ci.yml` clean, openssl 64-hex 확인, YAML 파싱 OK(env 에서 키 제거, step 순서 정합).
- 2026-06-30 **H 완료(DevOps 전달 — 코드범위 밖)** — 인프라 IaC 항목 enumerate(§H): (1) IDE pod **NetworkPolicy** — 셸 egress 차단(메타데이터 `169.254.169.254`/Redis/DB/타 pod), SSRF(P4 O7)·SA토큰(P4 O12)·securityContext(P4 O8)의 네트워크 측 보완. (2) prod **Environment required-reviewers**(C3 ticket 가드의 인적 승인 보완 — 코드로 강제 불가). (3) **OAuth secret 파일마운트**(C7) — 현재 env-var(K8s 표준, §6 수용), 파일마운트 전환은 선택. (4) **RBAC/PodSecurity** 정책. 모두 cloud-infra IaC 라 코드 PR 범위 밖 — DevOps 채널 전달이 완료조건. 전 Phase(P0~P7+H) 종료.
