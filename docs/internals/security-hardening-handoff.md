...
- **다음 작업 = P6** (ant-cloud deploy ticket 가드 + auth 디버그 로깅, 작업 디렉토리 `/Users/probe/dev/ant-cloud`, §5 참조). P6~P7 은 모두 ant-cloud.
- **완료**: P0·P1·P2·P3·P4 (ant 레포) + P5 (ant-cloud). 상세·검증결과는 §9 진행로그.
- **ant-cloud 커밋 상태** — `ant-cloud` `main` 직접, **미푸시**(push 는 사용자 확인 후): `b298795` P5 (workflows SHA 핀 + permissions + SECURITY.md).
...
- **이월/별도 레포 TODO**: ① ~~`ant-cloud/SECURITY.md`~~ **완료**(P5 `b298795` 에서 커밋). ② docker build 실증(P2, 로컬 데몬 down 으로 정적검증만) — 데몬 가용 시 cli/ui/ide 빌드 green 재확인 권장. ③ **P4 O8 securityContext 런타임 실증**: openvscode-server pod 가 uid1000 로 EFS subPath 쓰기 가능한지 — 정적 필드만 검증됨, 이미지 기본 user 가 1000 이라 무변경 기대이나 클러스터 미실증.
